...
import { firstSafeSemanticText, safeSemanticText, SEMANTIC_LIMITS } from "./semantic-boundary.mjs";

const scriptsDir = dirname(fileURLToPath(import.meta.url));

export async function applyRuntimeLedgerWriteback({ projectRoot, runtimeResult, envelope, snapshot, gate, dryRun = false }) {
  ...
  await runLedgerScript(root, ["project-state.mjs", "render", "arckit/project/state.record.json"]);
  changedFiles.push("arckit/project/STATE.md");
  if (iterationPath) {
    await runLedgerScript(root, ["project-iteration.mjs", "render", relativeToProject(root, iterationPath)]);
    await runLedgerScript(root, ["project-iteration.mjs", "index"]);
    changedFiles.push(iterationPath.replace(/\.record\.json$/, ".md").slice(root.length + 1));
    changedFiles.push("arckit/project/ITERATIONS.md");
  }
  await runLedgerScript(root, ["development-case.mjs", "index"]);
...
function runLedgerScript(projectRoot, args, { nodeBin = process.execPath } = {}) {
  const [script, ...rest] = args;
  const allowedScripts = new Set(["project-state.mjs", "project-iteration.mjs", "development-case.mjs"]);
  if (!allowedScripts.has(script)) {
    throw new Error(`Unsupported development-ledger script: ${script}`);
  }
  ...
  if (result.status !== 0) {
    throw new Error(`Development ledger script failed: ${basename(script)} ${rest.join(" ")}\n${result.stderr || result.stdout}`);
  }
  return {
