...
function createReferenceResolver({ command, project, record, issues }) {
  ...
  function resolveAssessmentGap(ref, path) {
    const parsed = parseRef(ref);
    if (parsed?.scope === 'local' && parsed.type === 'review-finding') {
      const findingId = resolve(ref, 'review-finding', path);
      return `${record.id}:review-finding:${findingId}`;
    }
    return resolve(ref, 'gap', path);
  }

  return { allocate, resolve, resolveAssessmentGap, mapping: () => Object.fromEntries([...local.entries()].map(([ref, value]) => [ref, value.id])) };
}
...
function materializeAssessment(assessment, { refs, project, issues }) {
  const judgments = assessment.judgments.map((item, index) => ({
    ...
    evidence: [...item.evidence],
    gap_refs: item.gap_refs.map((ref, refIndex) => refs.resolveAssessmentGap(ref, `case_command.invariant_assessment.judgments[${index}].gap_refs[${refIndex}]`)),
  }));
...
function projectedCaseSets(record, selectedGap, fresh, acceptedStateDelta) {
  ...
  for (const finding of acceptedStateDelta.completion_review_result?.findings || []) {
    const gapId = `${record.id}:review-finding:${finding.id}`;
    openGaps.add(gapId); allGaps.add(gapId);
  }
