...
- Transition `arckit-case-transition/v6`：一次 Gap 的显式选择依据、Case delta 与同一原子提交中的 Project delta。
...
- `software_invariants` 是对被接受 Case transition 的跨项目抽象正确性约束，不是工作发现器或 Gap 类型清单。六条核心不变量不可删除、任意改写或退役；协议升级只允许精确同步 canonical core 定义。项目仅可增加真正跨 Case 长期成立的非核心不变量。
...
- 每轮必须提交 `gap_selection`。`candidate` 逐字段复现并关闭当前派生候选；`fresh` 原子创建并关闭一个此前未持久化、Agent-owned、无未闭合依赖且本轮已完成的普通 Gap。`gaps_added` 只承接本轮已发现但仍真实未解决的工作，不用来预排未来 Loop。
