...
- 收到后续用户消息时，先分类 turn 类型，再决定是否重编 workflow frame、更新 workflow resolution、更新 artifact routing、生成 `workflow_correction_ledger` 或停止当前任务。
...
- 用户使用“后续/以后/当前项目/不要/只需要/必须/我来处理/为什么没有学习/为什么跳过”等表达，并改变 agent 的流程选择、验证方式、确认点、工具使用、停止条件、工作流复用方式、执行记录规则或术语边界时，默认判定为 `user_workflow_correction`。
...
输入：后续用户消息、当前或最近的 `workflow_frame`、`workflow_resolution`、已执行动作、已改文件、artifact impact scan 状态、memory overlay、execution record 状态和当前停止条件。

动作：
- 判断这条消息是否发生在首轮目标之后。
- 读取当前 workflow frame 中的 `workflow_resolution`、`final_goal`、`current_phase`、`selected_capabilities`、`confirmation_points`、`stop_conditions`、`artifact_targets`、execution record 目标和验证策略。
...
- 将后续消息分类为以下一种主类型：
  ...
  - `user_workflow_correction`：纠正流程选择、workflow resolution、场景工作流复用方式、execution record、验证方式、确认点、工具使用、停止条件、术语边界或记忆判断。
...
- `user_workflow_correction`：生成 `workflow_correction_ledger`，并要求 workflow frame 重新编译、更新 workflow resolution、更新 execution record 目标或调整停止条件。
