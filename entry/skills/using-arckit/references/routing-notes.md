...
当前仓库不承载具体技术栈或 agent skill 写作的正向实现 adapter。用户要求进入实现时，先形成统一的 `implementation_handoff`，标注 `artifact_type: code|skill|document|workflow|mixed|unknown`，再按对应 adapter 执行；实现改变范围、证据、计划或稳定文档时，再回到对应 ArcKit skill。

Agent skill 和 code 都是可能的实现产物。创建、维护、修复或验证一个 agent skill 能力时，前置流程仍按需求、设计、技术、迭代、治理和验证判断；最终进入实现、隔离试跑、同步或发布准备时，再交给 Skill First、skill creator 或 ArcForge 类 adapter。只有纯安装、同步、漂移检查、profile 归类、应用、正式化、共享或发布准备等已明确治理动作，且没有定义或实现不确定性时，才不强行进入完整 ArcKit workflow frame。
...
- `final_goal`：用户最终想得到什么，例如可用实现产物、产品判断、可确认原型、稳定规格、技术方案、修复结果、发布判断或工作流维护结果。
...
- 实现不是默认终点：正向实现前先判断目标实现产物是否建立、强化或改变产品、交互、视觉或技术规范。若 UI 一致性、跨页面行为/样式、组件状态或从实现中反推规范变化影响交互/视觉，先将 `interaction` 和 `visual` 标记为 `check`，实现后再依据证据决定是否 `update`。
...
- 最终产物是 code、agent skill、document、workflow 或 mixed artifact 时，都先判断用户目标、当前阶段、缺口、事实源影响、治理归属和验证口径。
- 需求、交互、视觉、技术方案、迭代、任务治理、评测和验证不因最终产物是 agent skill 而跳过。
- `artifact_type` 只用于选择实现 adapter、验证证据、发布/同步治理和收口字段，不用于提前绕过标准软件流程。

实现 adapter 选择：

- `artifact_type: code`：普通代码工作流或外部 `arckit-code`，再用测试、构建、运行日志和用户可见行为验证。
- `artifact_type: skill`：Skill First、skill creator 或 ArcForge 类实现/验证 adapter，再用真实任务试跑、触发边界、安装状态和治理 diff 验证。
- `artifact_type: mixed`：拆分实现 handoff，分别说明 code、skill、docs 或 workflow 的边界和证据。
- `artifact_type: unknown`：先补需求、技术或治理判断，不直接选择实现 adapter。

纯治理直达条件：

- 用户只要求安装、同步、漂移检查、profile 归类、应用、正式化、共享或发布准备已有 agent skill。
- 目标、来源、应用目标和确认边界已经明确。
- 没有需求定义、设计、技术方案、实现修改或验证策略缺口。

满足这些条件时，可直接交给 ArcForge 类治理能力，不必为了“软件项目协作”强行补完整前置流程。
...
- 方向变成可执行版本：缺定义先补定义；缺推进方式用 `arckit-project-governance-workflow`；最后形成带 `artifact_type` 的 `implementation_handoff`。
- 功能、页面或模块定义：产品行为写 `arckit-spec`；交互写 `arckit-interaction`；视觉写 `arckit-visual`；技术方案写 `arckit-tech`。
- 正向实现：先判断实现是否会建立或强化产品、交互、视觉或技术规范；形成 `implementation_handoff` 并标注 `artifact_type: code|skill|document|workflow|mixed|unknown`；再交给对应实现 adapter，例如普通代码工作流、外部 `arckit-code`、Skill First、skill creator 或 ArcForge 类能力；完成后用 `arckit-verify-implementation` 按产物类型收集证据。UI 一致性、跨页面行为/样式统一或组件状态统一需要在实现前标记 `interaction: check`、`visual: check`，实现后按证据决定是否 update 文档。
...
- `implementation_handoff` 至少包含目标最终产物类型、`artifact_type`、预期事实依据、影响范围、禁止触碰范围、实现 adapter、实现约束、验收口径、需要回查的事实源、需要保留的未决项、发布/同步治理影响和验证方式。
...
- 正向实现也进入 workflow frame：不要因为没有技术栈或 skill 实现 adapter 就完全跳出 ArcKit。
