...
| `acceptance` | 本 Gap 可共同决定、共同验收的结果集合；变化时说明原有义务的去向，不能藏入下游独立决定或静默缩减验收 |
| `prerequisites` | 执行本 Gap 所必需的局部前置与共享依赖，包括重要决定的事实依据；每项含 `statement`、`status: satisfied`、非空 `evidence` |
...
| `boundary_reason` | 为什么这样拆分或合并；重要决定是否依赖尚未回答的问题，剩余未知为何不影响本轮结论；验收变化时原义务如何处理 |
...
重新判断不重置既有义务。按 [gap-reasoning.md](gap-reasoning.md) 核对重要决定的依据、新证据对具体主张的影响，以及原 Gap 与本轮验收的对应关系。使用上述现有字段引用相关事实并说明变化；选择时报告已知依赖，提交时据实际证据更新，不把事后解释冒充开轮依据。
