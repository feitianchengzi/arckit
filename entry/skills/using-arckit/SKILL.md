...
- 用户质疑流程、表达“先/为什么没有/让我选择/不是这个意思/需要确认或原型”等纠偏信号时，必须视为 workflow-level correction，暂停当前执行假设并重新编译 workflow frame。
...
- 在 `after_context_read`、`before_edit`、`after_execution`、`before_final` 和 `user_workflow_correction` 等 reflection gates 重新判断 skill 组合和 artifact targets。
- 当触发 `user_workflow_correction` 时，输出本轮识别到的 workflow 偏差、新的 workflow frame、本轮恢复策略，以及是否需要记录 workflow signal；不要把它当作普通聊天反馈继续原路径。
