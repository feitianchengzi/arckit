...
- 当专家 skill 返回 `pvw_handoff` 时，把它翻译到最窄需要的 Backlog、Goal、Scope、Task、Review、Decision、Roadmap 或 document-map 更新中。
...
- 结构化字段名、ID、文件名、代码符号、YAML keys、JSON keys 和 workflow tokens 保持英文，例如 `goal_id`、`iteration_id`、`done_when`、`evidence`、`pvw_handoff`、`backlog_candidates`、`decision_candidates`。
...
| --- | --- | --- |
| 实体、公式、状态、不变量、业务词汇、政策边界 | `$domain-modeling` | decision candidates、capability impacts、scope risks、task candidates |
| 能力、用户故事、非能力、需求漂移、promotion rules | `$product-definition` | capability updates、backlog candidates、scope boundaries、decision candidates |
| 用户反馈、页面层级、信息优先级、原型 walkthrough、交互风险 | `$interaction-validation` | review evidence、backlog candidates、task candidates、decision candidates |
| 代码修改、测试、浏览器检查、发布准备 | 普通实现 workflow | task evidence、Review result、Roadmap adjustment |

专家输出不应成为平行项目计划。把它们的 `pvw_handoff` 折回 workflow 文档。
