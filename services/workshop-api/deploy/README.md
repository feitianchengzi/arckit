...
│   ├── deploy.sh                    # 生产环境部署脚本（待实现）
│   ├── .env.production              # 生产环境配置（不提交 Git）
│   └── env.production.example       # 配置示例
...
- **生产环境**：`deploy/prod/.env.production`
...
# 编辑 deploy/prod/.env.production 配置你的环境变量
```
...
2. **生产环境使用内网地址**：`.env.production` 中应配置 RDS 内网地址以提高性能
