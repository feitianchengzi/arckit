...
权限：必须是项目成员；如果任务没有分配执行者，任何项目成员都可以修改/删除；如果任务分配了执行者，只有管理员/所有者/执行者可以修改/删除
...
- 如果任务没有分配执行者（`executor_id` 为 `null`），任何项目成员都可以修改/删除
- 如果任务分配了执行者，只有管理员/所有者/执行者可以修改/删除
...
**权限规则**:
- 如果任务没有分配执行者（`executor_id` 为 `null`），任何项目成员都可以修改
...
- 多个查询参数可以组合使用，例如：`?project_id=1&updated_after=2024-01-01T12:00:00Z&father_id=0`
...
**权限规则**:
- 如果任务没有分配执行者（`executor_id` 为 `null`），任何项目成员都可以删除
- 如果任务分配了执行者，只有管理员/所有者/执行者可以删除
