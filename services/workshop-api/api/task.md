...
| priority | int | 否 | 优先级（0为最高；显式传 `null` 清空，不传则不更新） |
| tags | string | 否 | 标签（可选，用逗号分割） |
...
- `priority` 显式传 `null` 表示清空优先级；不传则保持不变
...
**描述**: 查询指定创建时间范围内命中过滤条件的任务，并围绕命中任务补全其所在一脉的父子层级结构。该接口要求必须提供时间范围，且 `start_time` 与 `end_time` 的间隔不能超过 100 天。
...
- 过滤条件只用于确定命中任务；返回结构会补全每个命中任务的全部上游父链和全部下游子树
- 补全出来的父节点、子节点不受时间范围和其他过滤参数限制，不计入 `total`
- `total` 表示匹配过滤条件的任务总数，不表示返回树中的全部节点数
- 多个命中任务的一脉结果会合并去重后按 `father_id` 组装层级；只有命中任务会触发下游子树补全，单纯作为上游补全出现的节点不会额外展开其他兄弟分支
