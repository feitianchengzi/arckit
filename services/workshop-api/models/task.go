...
type Task struct {
	...
	CreatorID    uint       `json:"creator_id" gorm:"not null;index"` // 外键：创建者ID
	ExecutorID   *uint      `json:"executor_id,omitempty" gorm:"index"` // 外键：执行者ID（可为空）
	...
	Creator   User     `json:"creator,omitempty" gorm:"foreignKey:CreatorID"` // 创建者
	Executor  *User    `json:"executor,omitempty" gorm:"foreignKey:ExecutorID"` // 执行者
}
