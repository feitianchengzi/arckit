...
Arckit 采用 Apache License 2.0 开源。仓库中的 ArcOrbit、Todo、Feedback Console 与 Workshop API 产品源码采用 PolyForm Perimeter License 1.0.1，允许个人、团队和企业内部使用及修改，但不允许对外提供竞争性产品或服务。Feedback Web SDK、iOS 示例和公共协议/文档继续采用 Apache-2.0。完整范围见[中文许可说明](LICENSING.zh-CN.md)或 [English Licensing Guide](LICENSING.md)。
...
| `services/workshop-api/` | Todo 与 Feedback 共用 Go API |
| `packages/feedback-sdk-web/` | Apache-2.0 的 Feedback Web SDK |
| `examples/feedback-ios/` | Apache-2.0 的 iOS 接入示例 |
| `runtime/arcorbit/` | ArcOrbit Desktop 与 Runtime |
| `docs/workshop/` | 可公开的 Workshop 产品与设计资料 |
...
| 执行平面 | Skill | 核心职责 |
| --- | --- | --- |
| Entry | [`using-arckit`](entry/skills/using-arckit/) | 同一 Agent 按完整状态驱动方法逐轮选择、执行并验证一个 Gap |
| Entry | [`arckit-development-ledger`](entry/skills/arckit-development-ledger/) | 状态模型、语义提交契约、可信记账与 fresh snapshot |

`using-arckit` 提供完整状态驱动方法；`arckit-development-ledger` 提供状态模型、语义提交契约、可信审计、原子提交与可恢复投影。两个 skill 分别通过 manifest 绑定，作为兼容集合随应用分发。Automation 默认提供两个入口；Chat 使用完整 Loop 时需让两个 skill 都可直接发现，单独使用 Ledger 可以进行显式状态维护。人工对话和 Runtime 使用同一条 Project State → Case → Loop 主轴。
...
安装 skills 后，可以让支持 Agent skills 的编码 Agent 使用 `using-arckit` 进入项目对话。同一 Agent 恢复 Project 和 active Case，选择一个 Gap，自主执行并验证，可信提交和 fresh-read 后进入下一轮。

需要单独维护稳定事实或进行诊断时，也可以直接触发相应 Worker skill。此时 skill 仍遵守自己的事实边界，但不会自行推断或写回 Case resolution。

### 通过 Desktop / Runtime 自动桥接

Runtime 在同一 Agent thread 中调用 `using-arckit`，通过 `arckit-development-ledger` capability manifest 调用 trusted entrypoints，串行推进 Gap、Review 和修复。Desktop 是推荐的可视化产品入口。
...
- Arckit 及仓库中没有更具体许可证的内容：[Apache License 2.0](LICENSE)（[简体中文参考译文](LICENSE.zh-CN.md)）。
- `runtime/arcorbit/`、`apps/todo-web/`、`apps/feedback-console/`、`services/workshop-api/`：[PolyForm Perimeter License 1.0.1](runtime/arcorbit/LICENSE)（[简体中文参考译文](runtime/arcorbit/LICENSE.zh-CN.md)）。
- `packages/feedback-sdk-web/`、`examples/feedback-ios/` 和公共 Workshop 文档：Apache License 2.0。
- 商业授权、OEM、白标或竞争性产品与服务：`hi@feitianchengzi.com`。
- 商标规则与第三方组件边界见[中文商标规则](TRADEMARKS.zh-CN.md)和[中文许可说明](LICENSING.zh-CN.md)。
