...
Arckit 采用“阶段或横向能力目录 / `skills` / 具体 skill”的结构。使用 SkillOps 时，可以让它递归扫描仓库中的 `SKILL.md`，并用 profile 控制不同项目需要启用哪些 skills。
...
如果后续某个项目只需要具体技术栈 coding skills，应从 `arckit-code` 选择；如果只需要 Arckit 中的生命周期、治理、诊断或交付 skills，应通过 SkillOps profile 做选择，而不是复制整个 Arckit 仓库。
