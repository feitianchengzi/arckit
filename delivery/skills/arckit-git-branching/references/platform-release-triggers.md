...
- 当前分支是所有普通 Git 操作的默认承载。用户没有明确要求分支变更时，必须保持当前分支。
- `git commit`、`提交这些改动`、`把改动提交一下` 只授权在当前分支 stage/commit；当前是 `main` 就提交到 `main`，当前是其他分支就提交到该分支。
- 只有用户明确要求或确认“创建、切换、合并、删除、回流、push 某分支”时，才能执行对应分支操作。目标分支不明确时先澄清。
- 授权按操作和目标生效：commit 授权不包含 branch create/switch，tag 授权不包含 branch push，先前发布讨论也不构成后续分支维护授权。
- Agent 可以因长周期、高风险或独立评审需求推荐 `feature/*`，但只能推荐；未获明确确认时继续保持当前分支。
