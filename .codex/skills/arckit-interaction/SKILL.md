...
description: "维护 arckit/interaction/ 下的页面级交互策略、灰度线框 HTML 和交互文档。适用于需要查询、创建、更新或审计稳定交互预期事实的场景。输出涉及的 interaction 路径、依据和摘要。"
---
...
本 skill 是结果型 skill，`arckit/interaction/` 仍是页面级交互事实的 source of truth。可以接收用户、Controller worker report、稳定产品事实或外部 adapter 提供的明确材料，但接收不等于自动写入。
...
Agent 判断当前 dynamic gap 需要查询或维护交互事实时，向本 skill 提供 `case_gap`。先读取 [`../_arckit_shared/case-gap-contract.md`](../_arckit_shared/case-gap-contract.md)，再基于完整上下文进入 managed-case 模式。实现代码、运行界面或测试只是相关证据；本 skill 维护稳定交互事实并返回 `fact_result`，不写 Case/Project State、不规定工作顺序。没有 `case_gap` 时使用 standalone 模式。
...
- **源缺失**：目标 `interaction.md` 没有「交互策略」章节。处理方式：先补足以支撑线框和规范投影的交互策略，再执行产物更新。
...
读取目标 `interaction.md` 的「交互策略」章节；若是新建页面，则先在模板中补齐足以支撑本次线框和规范投影的策略。判定本次请求是投影变更、源变更还是源缺失，并据此决定是否先更新策略源。
