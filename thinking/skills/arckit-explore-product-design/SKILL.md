...
description: 在写入正式交互或视觉结果前探索产品设计方向、页面方案、状态表达、交互风险、AI 默认设计问题或候选方案对比。适用于需要设计探索 handoff、候选方案比较或为交互/视觉事实源提供输入的场景。输出设计探索 handoff，不直接维护 arckit/interaction 或 arckit/visual。
---
...
- 不适合：替人类裁决审美高低、品牌方向、商业优先级或最终体验取舍；这类内容只能形成候选、风险、证据和待确认问题。
...
5. 标记哪些判断需要人类确认，哪些可以作为已确认交互事实或视觉事实候选。
6. 输出可交给结果 skill 的 `design_exploration_handoff`。
