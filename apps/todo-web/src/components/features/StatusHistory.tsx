  ...
  if (!history || history.length === 0) {
    return (
      <div className={className}>
        <h3 className="text-lg font-semibold text-foreground mb-4">状态历史</h3>
        {lastUpdatedAt && (
        ...
        {history.map((item, index) => (
          <div key={item.id} className="relative pl-8 pb-4">
            ...
            {index < history.length - 1 && (
              <div className="absolute left-3 top-6 bottom-0 w-0.5 bg-divider" />
            )}
            
            {/* 时间点 */}
            <div className="absolute left-0 top-1 w-2 h-2 rounded-full bg-primary border-2" style={{ borderColor: 'var(--color-surface-elevated)' }} />
            
            {/* 内容 */}
            <div className="space-y-1">
              <div className="flex items-center gap-3">
                <StatusBadge status={stateToStatus(item.old_state)} size="sm" />
                <ArrowIcon className="w-4 h-4 text-foreground-tertiary" />
