...
import type { CreateTaskInput } from '@/lib/api/endpoints/tasks'
...
export interface CreateTaskDialogProps {
  ...
  onCreate?: (input: CreateTaskInput) => Promise<{ id: number }> // 可选的专用创建流程
  initialContent?: string // 预填内容（可选）
  initialPriority?: number | null // 预填优先级（可选）
}
...
export function CreateTaskDialog({
  ...
  }, [open, parentId, initialContent, initialPriority])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newTask = onCreate ? await onCreate(taskInput) : await createTask.mutateAsync(taskInput)
