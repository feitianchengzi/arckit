...
export function useCreateProject() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      // 跳转到项目详情（feedback 区域保持在 feedback 路由）
      navigate(isFeedbackSection ? buildFeedbackProjectPath(data.id) : buildProjectPath(data.id))
    },
  })
}
...
export function useDeleteProject() {
  ...
  return useMutation({
    ...
    onSuccess: (_, projectId) => {
      ...
      // 跳转回项目列表（feedback 区域保持在 feedback 路由）
      navigate(isFeedbackSection ? '/feedbacks' : '/projects')
    },
  })
}
