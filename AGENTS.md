...
- State Driven Loop method: `entry/skills/using-arckit/`
- Trusted development ledger and semantic submission contracts: `entry/skills/arckit-development-ledger/`
...
- Runtime capability binding: `runtime/arcorbit/config/capability-policy.json`, which manages only the `using-arckit` Agent entry and `arckit-development-ledger` trusted Runtime entrypoints. Automation Git delivery uses the Agent's native ability in the same conversation and adds no capability binding or skill dependency. Definition, diagnosis, code, quality, and user-installed skills remain available through Codex native skill discovery inside the coherent Agent thread; Runtime does not maintain a Worker registry.
