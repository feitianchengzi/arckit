...
- 如果本轮目标项目可稳定指纹化，必要时初始化 `~/.arckit/workflows/projects/<project-fingerprint>/{INDEX.md,signals,candidates,accepted}`。
...
输入：最终 workflow frame、实际使用的 skills、文件、命令、写入路径、验证结果、用户反馈、失败点、`workflow_correction_ledger`。
