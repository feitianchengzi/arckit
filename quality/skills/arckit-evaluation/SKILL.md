...
本 skill 维护真实研发活动评测集。它把用户输入、模拟场景、试跑偏差和覆盖结论保存在 `arckit/evaluation/`，用于验证 Arckit 产品方案、skill 体系和不同实现产物是否覆盖真实软件研发活动。
...
- 评测流程不按 code、skill 或其他最终产物类型分裂；产物类型只用于记录验收对象、实现证据和失败判断差异。
...
- 明确评测对象：产品方案、入口路由、单个 skill、多个 skills 工作流、实现产物或工作方式。
- 明确最终产物类型：`code`、`skill`、`document`、`workflow`、`mixed` 或 `unknown`；不要用该字段替代真实研发活动和能力组合判断。
- 明确本轮禁止触碰范围，例如不改 code、不改 skill、不改 spec、不改 tech。

退出条件：评测目标、对象、最终产物类型、真实研发活动和写入边界明确。
...
- 每个场景至少包含：场景编号、场景名称、最终产物类型、用户输入样本、真实研发活动、本轮真实目标、期望能力组合、期望产物、期望沉淀路径、验收重点、产物类型相关证据、当前风险或疑问、评测状态。
