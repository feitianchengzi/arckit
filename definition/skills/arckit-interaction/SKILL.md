...
维护页面级交互设计：用交互策略定义预期行为，用可交互原型呈现操作体验，用说明解释路径、状态与边界，三者保持一致。遵循 INDEX 驱动、页面归属和渐进式披露；产品上线后继续维护这套设计，生产实现提供核对依据。
...
读取 `arckit/interaction/INDEX.md`、目标页的策略与现有原型、相关产品事实和适用视觉规范。变更前读 [索引与拆分](references/index-analysis.md)。已有页面默认并入；跨页变更结合共享资源和各页面行为确定维护范围，记录导航关系。`_explorations/`、`_shared/`、`_map/`、`_archive/` 是基础设施，不是业务页面。
探索或采纳时读取 [探索与采纳](references/exploration.md)；先确定候选基线与写入范围，再创建产物。未确认事项可以形成探索材料，不能为此强行建立正式页面。

### 3. 确认策略与视觉依据

正式维护先判断预期行为是否变化：
- 主任务、路径、状态流、反馈或边界变化时，明确新的交互策略，据此更新原型与说明。
- 预期行为不变时，保留策略，完善需要调整的原型呈现或说明。
- 策略缺失时，先补足有依据的预期；缺少依据时保留问题，不编造确认。
