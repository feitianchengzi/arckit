...
1. 在 `interaction.md` 填写「交互策略」：核心任务、用户场景、主路径、决策点、信息揭示、状态流、反馈与恢复、输入输出边界。策略只写当前有效约束，不写方案讨论过程。
2. 判断是否为复杂流程：若包含 3 个及以上任务阶段/决策点、Sheet/Alert/权限/长任务、错误恢复/返回修正/稍后继续、多来源/批处理/支付/生成/导入导出等高风险操作，按复杂流程处理。
3. 编辑 `default.html` 填充各状态的 UI 内容，确保线框是交互策略的投影
   ...
   - 简单页面至少 4 个状态：加载中 / 成功 / 空状态 / 错误
   - 复杂流程必须按真实任务阶段拆成独立 `<details>` 状态块；例如入口、选择、预览、修正、执行、完成、失败恢复、权限恢复
   - 每个状态必须包含 `触发条件`、`组件清单`、`交互行为`
   - 禁止把复杂流程的多个阶段只压成同一个 device-frame 下的连续 section；除非交互策略明确说明这些阶段是同屏渐进揭示
   - 弹窗直接渲染在页面中（.modal-overlay + data-kit="Sheet|Alert|..."），弹窗占位也放在 device-frame 内
   - Sheet / Alert / overlay 状态过多时，拆出 `overlays.html` 或子流程 HTML，并在 INDEX 与 `interaction.md` 中引用
4. 若线框图使用新类名或新组件样式，在 `arckit/interaction/wireframe-style.css` 中补充对应样式：须**抽象为通用组件**（不与具体业务耦合）、**扩写前搜索**避免重复定义；详见 [wireframe-style.md](wireframe-style.md)「业务开发中的维护」与「扩写规则」。
5. 补充 `interaction.md` 的状态、导航、交互行为、弹窗、错误处理、加载策略，确保规范章节是交互策略的投影
6. 执行投影一致性自查：线框和规范是否共同体现同一主路径、状态流、反馈机制和异常恢复；复杂流程还必须检查状态投影覆盖率和逐状态可执行性。
7. 更新 `arckit/interaction/INDEX.md` 添加条目（含行数如 `(120行)`）
8. 更新 `arckit/_map/RELATIONS.md` 和 `feature-matrix.md`

**或手动创建**（不用脚本）：

1. 确定所属流程（若不存在 → 创建目录）
2. 创建 interaction.md，并先填写「交互策略」
3. 判断复杂度；复杂流程先列出关键任务阶段，再创建 default.html
4. 创建 default.html；复杂流程按关键阶段生成独立 `<details>` 状态块，每个状态包含触发条件、组件清单、交互行为
5. 参考 references/data-kit-mappings.md 标注 data-kit 属性
6. 若使用新类名或新组件样式，在 interaction 根目录 wireframe-style.css 中补充对应样式（须为通用组件、无重复定义，见 wireframe-style.md 扩写规则）
7. 补齐交互规范章节并执行投影一致性自查
8. 更新 INDEX.md（添加条目+初始行数 如 `(120行)`）
9. 更新 _map/RELATIONS.md、feature-matrix.md
...
6. 执行投影一致性自查：`default.html` 与 `interaction.md` 是否共同表达同一交互策略
   - 复杂流程：关键任务阶段是否在 HTML 中可见
   - 每个状态是否有触发条件、组件清单、交互行为
   - Sheet / Alert / overlay / 恢复路径是否可见；若拆分，引用是否完整
