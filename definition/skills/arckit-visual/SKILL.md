...
本 skill 是结果型 skill，`arckit/visual/` 仍是视觉策略、Design Tokens、主题和组件视觉规格的 source of truth。可以接收 `arckit-explore-product-design`、`arckit-decision-framework` 或 `arckit-project-governance-workflow` 的 handoff，但接收不等于自动写入。
