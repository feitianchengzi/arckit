...
test("distribution assembly binds provider, skills, trusted capabilities, config, and external artifact attestation", async () => {
  ...
  try {
    ...
    const internalManifest = { schemaVersion: "arcforge-provider-package/v1", apiVersion: "arcforge-embedded-provider/v1", providerVersion: "0.1.0-tf.b1", buildCommit: "a".repeat(40), releaseTag: "tf/v0.1.0-b1", capabilities: ["declared-shared-assets/v1", "source-upgrade-recovery/v1", "conflict-reinstall-recovery/v1", "stable-catalog/v1", "project-skill-migration/v1"], entrypoint: "dist/provider/index.js", loaderPath: "skills/arcforge-on-demand" };
    ...
    assert.deepEqual(lock.arcforgeProvider.capabilities, ["declared-shared-assets/v1", "source-upgrade-recovery/v1", "conflict-reinstall-recovery/v1", "stable-catalog/v1", "project-skill-migration/v1"]);
