...
const snap=()=>({account_scope:'fixture-user',user:{id:'7'},projects:[{id:'p',name:'ArcOrbit',local_project_id:'local',state:'healthy'},{id:'q',name:'协作平台',local_project_id:'local-q',state:'healthy'}],local_projects:[{id:'local',name:'ArcOrbit'}],tasks,scenes,runtime:{queue:[{id:'3',task_id:'3'}],attention_items:[{task_id:'2',reason:'待验收'}],recovery_items:[],active_executions:[{task_id:'4',execution_id:'e4',phase:'running'}]},settings:{codex:{chat:{model:'gpt-6-astra',reasoning_effort:'high'}}},source_status:'healthy',errors:[]});
ipcMain.handle('pw:snapshot',()=>snap());
ipcMain.handle('pw:detail',(_event,id)=>({task:tasks.find(t=>t.id===id),scene:scenes[id],local_project:{id:'local',name:'ArcOrbit'},session:null,messages:messages[id]||[],runs:[],activity:null,result:null,executions:[],history:[],attachments:[],attachment_error:'',feedback:[],recovery:[],attention:[],current_turn_owner:'',children:[],user:{id:'7'},members:[]}));
...
try{
 ...
 checks.push('all eleven legacy pages remain reachable');await js(`document.querySelector('[data-page="project-workbench"]').click()`);await wait(`document.querySelector('.pw-task-row[aria-selected=true]')`,'return');
