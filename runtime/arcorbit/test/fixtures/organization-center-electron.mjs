...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const result = await window.webContents.executeJavaScript(`(async () => {
      ...
      const executorAutomationHelp = () => document.querySelector('[name="executor_id"]')?.closest('.platform-action-field')?.querySelector('small')?.textContent || '';
      const initialExecutorAutomationHelp = executorAutomationHelp();
      ...
      const executorGuidanceCallCountBefore = (await window.arckitDesktop.getTestCalls()).length;
      ...
      document.querySelector('[name="executor_id"]').dispatchEvent(new Event('change', { bubbles: true }));
      const currentUserReviewAutomationHelp = executorAutomationHelp();
      document.querySelector('[name="state"]').value = 'pending';
      document.querySelector('[name="state"]').dispatchEvent(new Event('change', { bubbles: true }));
      const currentUserPendingAutomationHelp = executorAutomationHelp();
      document.querySelector('[name="executor_id"]').value = '8';
      document.querySelector('[name="executor_id"]').dispatchEvent(new Event('change', { bubbles: true }));
      const otherUserPendingAutomationHelp = executorAutomationHelp();
      document.querySelector('[name="project_id"]').value = '12';
      document.querySelector('[name="project_id"]').dispatchEvent(new Event('change', { bubbles: true })); await wait();
      const productSwitchAutomationHelp = executorAutomationHelp();
      const executorGuidanceCallCountAfter = (await window.arckitDesktop.getTestCalls()).length;
      document.querySelector('[name="project_id"]').value = '11';
      document.querySelector('[name="project_id"]').dispatchEvent(new Event('change', { bubbles: true })); await wait();
      document.querySelector('[name="state"]').value = 'pending_review';
      document.querySelector('[name="state"]').dispatchEvent(new Event('change', { bubbles: true }));
      ...
      document.querySelector('#platformWorkLayout').style.setProperty('--work-inspector-width', '360px');
      await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
      const acceptanceItem = document.querySelector('#platformWorkInspector .acceptance-feedback-item');
      const acceptanceCopy = acceptanceItem.querySelector(':scope > span:first-child');
      const acceptanceTitle = acceptanceCopy.querySelector('strong');
      const acceptanceProgress = acceptanceCopy.querySelector('small');
      const acceptanceStatus = acceptanceItem.querySelector('.status-pill');
      const itemRect = acceptanceItem.getBoundingClientRect();
      const copyRect = acceptanceCopy.getBoundingClientRect();
      const statusRect = acceptanceStatus.getBoundingClientRect();
      const acceptanceFeedbackLayout = {
        item_overflows: acceptanceItem.scrollWidth > acceptanceItem.clientWidth,
        copy_overflows: acceptanceCopy.scrollWidth > acceptanceCopy.clientWidth,
        copy_within_item: copyRect.left >= itemRect.left && copyRect.right <= itemRect.right,
        status_within_item: statusRect.left >= itemRect.left && statusRect.right <= itemRect.right,
        title_lines: Math.round(acceptanceTitle.getBoundingClientRect().height / parseFloat(getComputedStyle(acceptanceTitle).lineHeight)),
        progress_lines: Math.round(acceptanceProgress.getBoundingClientRect().height / parseFloat(getComputedStyle(acceptanceProgress).lineHeight)),
        title_white_space: getComputedStyle(acceptanceTitle).whiteSpace,
        progress_white_space: getComputedStyle(acceptanceProgress).whiteSpace,
        title_overflow_wrap: getComputedStyle(acceptanceTitle).overflowWrap,
        progress_overflow_wrap: getComputedStyle(acceptanceProgress).overflowWrap
      };
      ...
      const memberProjectHasBindingAction = Boolean(document.querySelector('[data-organization-bind-workspace="12"]'));
      const memberProjectHasParticipationAction = Boolean(document.querySelector('[data-organization-enable-project="12"]'));
      click('[data-organization-bind-workspace="12"]'); await wait();
