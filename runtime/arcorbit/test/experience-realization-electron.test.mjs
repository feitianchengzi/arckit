...
assert.equal(result.standard_control_violations, 0, JSON.stringify(result.control_violation_details));
...
const expectedNavigation = ["today", "chat", "project-workbench", "product", "idea", "work", "command", "release", "operations", "feedback", "organization", "engineering"];
assert.deepEqual(result.core_navigation_pages, expectedNavigation);
assert.equal(result.core_navigation_vector_icons, expectedNavigation.length);
...
assert.equal(result.today.sheet_title, "管理 核心推进");
assert.deepEqual(result.today.workset_project_options, ["11", "12", "21"]);
assert.deepEqual(result.today.selected_workset_projects, ["11", "12"]);
