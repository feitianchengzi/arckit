...
test("default execution accepts candidate and current-turn fresh gaps from one coherent using-arckit Agent", async () => {
  ...
  const invocation = JSON.parse(await readFile(contextPath, "utf8"));
...
function agentLoopResult(gap, caseId, mode = "candidate") {
  return {
    ...
    changed_files: ["runtime/arcorbit/src/agent-orchestrator.mjs"],
    artifact_impacts: [{
      path: "runtime/arcorbit/src/agent-orchestrator.mjs",
      claim: "updated",
