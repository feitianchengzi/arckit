...
test('primary navigation contains one legacy menu with every former main page',async()=>{
 const html=await readFile(new URL('../desktop/renderer/index.html',import.meta.url),'utf8');const legacy=html.split('id="legacyPagesMenu"')[1].split('id="projectWorkbenchNav"')[0];for(const id of ['today','chat','product','idea','work','command','release','operations','feedback','organization','engineering'])assert.match(legacy,new RegExp(`data-page="${id}"`));
