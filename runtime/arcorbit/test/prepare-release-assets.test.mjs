...
test("release assets combine target checksums into one deterministic manifest", async () => {
  ...
  try {
    const targets = [
      ["windows", "ArcOrbit-win.exe", "windows installer"],
      ["linux", "ArcOrbit-linux.AppImage", "linux installer"]
    ];
    ...
    assert.deepEqual(result.checksums, [
      `${sha256("linux installer")}  ArcOrbit-linux.AppImage`,
      `${sha256("windows installer")}  ArcOrbit-win.exe`
    ]);
