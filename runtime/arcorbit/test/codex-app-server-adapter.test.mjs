...
test('scene roots are configured once per process and exclusions travel with thread resume', async () => {
 ...
 client.request=async(method,params)=>{
  ...
  if(method==='skills/list'){client.requests.push({method,params});return {data:[{cwd:'/workspace/project',skills:[{name:'using-arckit',path:'/legacy/using-arckit/SKILL.md',enabled:true}]}]};}
  return request(method,params);
 };
 const body={schema_version:'arcorbit-scene-skill-binding/v1',scene:'chat',skills:[],managedNames:['using-arckit'],disabledPaths:[]};
