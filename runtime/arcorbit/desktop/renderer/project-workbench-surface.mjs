...
export function createProjectWorkbenchSurface({root,nav,api,navigate,openSettings}) {
  ...
  root.innerHTML=`<div class="pw-shell"><header class="pw-top"><strong data-pw-heading>项目事情台</strong><select class="pw-scope-select" aria-label="项目范围"></select>${button('sync','↻','title="同步事情"')}${button('list','事情列表','','pw-list-toggle')}${button('runtime','运行状态','aria-expanded="false" aria-controls="pw-runtime"')}</header>
    ...
    <aside class="pw-list" aria-label="事情列表"><header class="pw-list-head"><div class="pw-list-title"><strong>事情 <small data-pw-count>0</small></strong><select data-filter="state" aria-label="事情状态">${options([['','全部状态'],...Object.entries(stateLabels)],'')}</select>${button('filters','筛选','','quiet small')}${button('list.close','×','aria-label="关闭事情列表"','quiet pw-list-close')}</div><input class="pw-search" placeholder="搜索事情或编号" aria-label="搜索事情"><div class="pw-extra" hidden><select data-filter="executor" aria-label="执行人"></select><select data-filter="priority" aria-label="优先级">${options([['','全部优先级'],['0','无优先级'],['1','P1'],['2','P2'],['3','P3']],'')}</select></div></header><div class="pw-rows"></div><footer class="pw-list-foot">${button('create','＋ 发起事情','','primary')}</footer></aside>
    <aside id="pw-runtime" class="pw-runtime" hidden aria-label="本机运行状态"></aside></div>`;
  ...
  function render(){
    ...
    if(!state.snapshot){paint(q('.pw-heading'),'<h1>项目事情台</h1>');paint(q('.pw-body'),`<div class="pw-empty">${state.error?'无法载入事情，请重试。':'正在同步项目与事情…'}${button('refresh','重新加载')}</div>`);return;}
    const s=state.snapshot,t=state.detail?.task,projects=s.projects||[],selected=projects.find(p=>p.id===state.project);
    const scopeOptions=[['all','全部事情'],['attention','需要关注'],...projects.map(p=>[p.id,p.name])];paint(q('.pw-scope-select'),options(scopeOptions,state.project));
    paint(nav,`<p>项目</p>${scopeOptions.map(([id,name])=>`<button type="button" data-project="${e(id)}" aria-current="${id===state.project}" title="${e(name)}">${e(name)}</button>`).join('')}`);
    q('[data-pw-heading]').textContent=selected?.name || (state.project==='attention'?'需要关注':'项目事情台');
    ...
    const counts=Object.fromEntries(groups.map(g=>[g.group,g.items.length]));const runButton=q('[data-pw-action=runtime]');runButton.textContent=counts.attention?`${counts.attention} 待处理 · ${counts.running} Auto`:`${counts.running} 正在 Auto${counts.queued?` · ${counts.queued} 排队`:''}`;runButton.setAttribute('aria-expanded',String(state.runtime));
    paint(q('.pw-runtime'),`<header><strong>此设备 · 所有项目</strong>${button('runtime.close','×','aria-label="关闭运行状态"','quiet small')}</header>${groups.filter(g=>g.items.length).map(g=>`<h3>${{attention:'需要处理',running:'正在运行',queued:'等待执行'}[g.group]} · ${g.items.length}</h3>${g.items.map(i=>`<button type="button" class="pw-runtime-item" data-pw-action="runtime.select" data-id="${e(i.id)}"><span>${e(title(i.task))}</span><small>${e(projects.find(p=>p.id===String(i.task.project_id))?.name||'')} · ${e(i.reason)}</small></button>`).join('')}`).join('')||'<div class="pw-empty">当前没有 Auto 执行</div>'}${button('settings','执行设置','','quiet')}`);
  ...
  async function project(id){saveScroll();state.project=id;state.filter='';state.search='';state.executor='';state.priority='';state.task='';state.detail=null;state.chat=false;q('.pw-search').value='';q('[data-filter=state]').value='';q('[data-filter=priority]').value='';selectionEpoch++;render();await api.projectWorkbenchCommand('sync',{project_id:['all','attention'].includes(id)?'':id});await refresh();persist();}
  ...
  function createTask(asChat=false,parent=''){
    ...
    showDialog(parent?'创建子事情':asChat?'发起事情并讨论':'发起事情',`<label>所属项目<select name="project_id">${options(s.projects.map(p=>[p.id,p.name]),projectId)}</select></label>${field('目标或问题','content',asChat?q('.pw-composer textarea').value:state.newDraft?.content||'','textarea')}<p class="pw-note">创建为待评审，执行人是你；Auto 由你单独发起。</p><label><input name="chat" type="checkbox" ${asChat?'checked':''}> 创建后与 Agent 讨论</label>`,async form=>{
      ...
      state.project=String(form.project_id);await select(created.task_id);state.newDraft=null;
  ...
  async function handle(action,target){const id=target.dataset.id,d=state.detail;
    ...
    if(action==='filters'){q('.pw-extra').hidden=!q('.pw-extra').hidden;return;}
    if(action==='list'||action==='list.close'){state.list=action==='list'?!state.list:false;render();return;}
    if(action==='runtime'||action==='runtime.close'){state.runtime=action==='runtime'?!state.runtime:false;render();return;}
    if(action==='runtime.select'){const t=state.snapshot.tasks.find(t=>String(t.id)===id);state.runtime=false;state.project=String(t.project_id);state.filter=state.search=state.executor=state.priority='';q('.pw-search').value='';q('[data-filter=state]').value='';await select(id);return;}
    if(action==='settings'){state.runtime=false;openSettings();return;}
  ...
  root.addEventListener('change',event=>{const n=event.target;if(n.matches('.pw-scope-select'))void act(()=>project(n.value));if(n.dataset.filter){state[{state:'filter',executor:'executor',priority:'priority'}[n.dataset.filter]]=n.value;render();persist();}if(n.dataset.criterion)void act(()=>invoke('criteria.check',{id:n.dataset.criterion,checked:n.checked}));if(n.dataset.context)void act(()=>invoke('context.include',{id:n.dataset.context,included:n.checked}));});
  ...
  nav.addEventListener('click',event=>{const target=event.target.closest('[data-project]');if(target){navigate('project-workbench');void act(()=>project(target.dataset.project));}});
  q('.pw-scrim').addEventListener('click',closeChat);
  document.addEventListener('keydown',event=>{if(event.key!=='Escape'||dialog||!state.active)return;if(state.runtime){state.runtime=false;render();}else if(state.list){state.list=false;render();}else closeChat();});
