...
export function createConversationComposer({input,sendButton,stopButton,hint,onInput,onSend,onStop,performAction=fn=>fn()}) {
  ...
  return {render({draft='',hasContext=false,available=true,active=false,sending=false,stopping=false,waiting=false,placeholder='说明你希望 Agent 帮你完成什么…'}){
    if(input.value!==draft)input.value=draft;input.disabled=!available;input.placeholder=placeholder;
    sendButton.disabled=!available||active||sending||(!draft.trim()&&!hasContext);sendButton.classList.toggle('hidden',active);
