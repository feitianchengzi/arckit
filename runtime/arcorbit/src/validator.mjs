...
function validateCaseControlHandoff(handoff, issues) {
  // Validate transport only. The manifest-bound trusted Ledger owns action and
  // semantic validation, including refs, evidence, policy and closed Case reuse.
  requireString(handoff.schema_version, "case_control_handoff.schema_version", issues);
  requireString(handoff.action, "case_control_handoff.action", issues);
