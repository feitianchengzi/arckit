...
async function runSameThreadCloseout({
  adapter,
  projectRoot,
  originalTask,
  threadKey,
  ...
  const prompt = [
    ...
    JSON.stringify({
      ...
      instruction: "The Case already passed Completion Review. Perform Git-only closeout in this same conversation: create one intentional, coherent commit containing all already-reviewed changes produced for this todo, or return no_changes when that work is already committed or there is genuinely nothing in scope. Treat the todo scope positively: its implementation, tests, durable product/interaction/visual/technical documentation, and canonical Arckit ledger artifacts written while advancing the Case normally belong together. Canonical ledger artifacts can include the active or closed Case file, Project or Iteration records and projections, and their indexes. Use the authoritative Case id, trusted ledger changed-files context, same-thread history, git status, diffs, and recent commits together to recognize that work. The provided ledger paths are strong context, not an exclusive allowlist: include related files or related hunks that complete the same reviewed todo, and preserve pre-existing changes that are genuinely unrelated. A file may contain both related and unrelated hunks; separate them when practical instead of excluding the whole file. Do not omit a current-todo artifact merely because it is Arckit metadata or was generated by trusted ledger writeback. Do not inspect semantic correctness, run validation, edit files, or repair content. If safe commit scope requires a content change or new semantic judgment, return needs_human.",
