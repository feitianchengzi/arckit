...
async function defaultRepositoryCapabilityRoot() {
  ...
  const packagedRoot = await resolvePackagedCapabilityRoot(resourcesPath);
  return packagedRoot || developmentRepositoryRoot;
}

export async function resolvePackagedCapabilityRoot(resourcesPath, { accessFile = access } = {}) {
  const packagedRoots = resourcesPath
    ? ["arcorbit", "arckit-runtime"].map((name) => resolve(resourcesPath, name, "trusted-capabilities"))
    : [];
