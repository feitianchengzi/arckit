...
test("Windows npm command shims are version-probed through a structured PowerShell boundary", () => {
  ...
  assert.equal(spec.env.ARCKIT_CODEX_PROBE_COMMAND, command);
  assert.equal(spec.env.ARCKIT_CODEX_PROBE_ARGS, JSON.stringify(["--version"]));
  assert.equal(spec.args.includes(command), false);
  assert.match(spec.args.at(-1), /ARCKIT_CODEX_PROBE_COMMAND/);
});
