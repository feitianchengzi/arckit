...
Empty projects are valid inputs. Adding a project initializes only `arckit/project`. The Controller decides whether the operator request belongs to an existing active Case or needs a new bounded Case; Runtime then invokes the trusted `case_control` entrypoint, and the ledger allocates the Case id and atomically selects it. Runtime does not invent a neutral Case, parse task keywords, or choose a first business gap.
