...
const trustedRoot = path.join(resourcesRoot, "arckit-runtime", "trusted-capabilities");
...
await writeFile(path.join(payloadRoot, "arcforge.config.json"), `${JSON.stringify({
  version: 1,
  sourceDir: ".",
  profiles: [{ name: "arckit-runtime", description: "Complete Arckit skill payload supplied by Arckit Runtime.", skills: ["*"], targets: ["codex"] }]
}, null, 2)}\n`);
...
const lock = {
  ...
  artifact: { expectedNamePattern: `Arckit-Runtime-${release.packageVersion}-${options.target}.*`, sha256: null, attestation: "external distribution-attestation.json binds the final artifact digest to this embedded lock digest" }
};
