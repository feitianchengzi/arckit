...
const config = {
  ...
  productName: "Arckit Runtime",
  executableName: "arckit-runtime",
  artifactName: `Arckit-Runtime-${lock.runtime.packageVersion}-${lock.runtime.channel}-${lock.runtime.buildLabel}-${"${os}"}-${"${arch}"}.${"${ext}"}`,
  directories: { output: "release", buildResources: "build" },
  asar: true,
  files: ["package.json", "bin/**/*", "adapters/**/*", "config/**/*", "desktop/**/*", "schemas/**/*", "src/**/*", "README.md"],
  extraResources: [
    { from: path.join(resourcesFrom, "arckit-runtime"), to: "arckit-runtime", filter: ["**/*"] },
