...
- 数据表和运行阶段使用 12–14px 的可读紧凑密度，正文与人工对话使用 14–15px 的舒适行高；11px 只用于短计数与非关键标识。
- 常规控件不低于 36px，可操作数据行不低于 40px，仅图标工具不低于 32px。
- 所有交互控件提供可见 focus 状态，正文与背景满足 WCAG AA 对比度，控件边界与焦点相对相邻颜色不低于 3:1。
- 指标卡只用于可比较且直接影响判断的指标；常规信息使用共享表面、分隔线和空间层级。
- 操作图标来自统一矢量体系，字符和 Emoji 不承担生产操作图标职责。
