...
Arckit 支持基于 skill 的动态工作流编排。该能力使 `using-arckit` 不只作为最小 skill 路由器存在，而是承担软件项目协作首轮场景识别、工作流匹配和 workflow frame 编译入口职责；任务执行中的后续用户消息由 `arckit-turn-adaptation` 分类和适配。
...
- 让 `arckit-turn-adaptation` 区分首轮之后的补充信息、流程纠偏、目标变更、项目事实纠正、澄清回答和暂停/停止。
...
Turn Adaptation 是 `arckit-turn-adaptation` 的职责。它处理首轮目标之后的用户消息，先判断消息类型，再决定是否更新 workflow frame、artifact routing、停止当前任务或生成 workflow correction ledger。
...
Workflow Memory 不负责判断一条原始用户消息是补充信息、目标变更还是流程纠偏。`using-arckit` 只负责识别首轮入口和后续消息交接；首轮之后的消息分类由 `arckit-turn-adaptation` 完成。Workflow Memory 消费最终 workflow frame、执行证据和可选的 `workflow_correction_ledger` 做 signal decision。
...
6. 设置 reflection gates，并在需要时把后续用户消息交给 `arckit-turn-adaptation`。
...
入口不默认加载所有 skill，也不把所有请求强行纳入单一固定流程。入口负责让首轮任务有明确的场景分类和工作流结构；后续消息分类、ledger 生成、signal decision 和持久化由对应专门 skill 处理。
...
当用户在首轮目标之后继续发消息时，系统必须先判断是否属于 turn adaptation，而不是把所有内容继续塞回 `using-arckit`。
...
3. 对补充信息、澄清回答、目标变更、项目事实纠正和暂停/停止分别给出适配动作。
...
- `turn_adaptation`：首轮之后用户补充、纠错、换目标、事实纠正、回答澄清或暂停时，交给 `arckit-turn-adaptation` 分类并决定是否重编 frame。

## 7. 自然调整

用户可以用自然语言调整当前 workflow。首轮之后的调整由 `arckit-turn-adaptation` 先分类。系统支持以下调整类型：
