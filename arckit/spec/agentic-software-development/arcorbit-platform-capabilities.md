...
各页面共用顶部观察范围并保留每条数据的产品归属。Today 项目栏始终展示当前工作集全部成员，局部项目选择独立于顶部观察范围；其他页面（包括 Chat、Thing、Product/Idea）按顶部范围投影。观察范围统一改变各页的数据投影与对象选择，不改变自动领取资格、项目成员关系、本地绑定、运行中的任务或持久 Codex thread。
...
Today 项目列表以当前产品集成员配置为唯一成员来源，不以已有 Today 配置、本地绑定或 Automation participation 筛除项目。产品集为空时显示成员管理引导；成员增删后列表随之更新，仍有效的局部选择保持，失效选择回到全部项目。

Today 点击项目只切换本页项目及对应内容，不修改顶部查看项目范围。顶部主动切换单项目或全部项目时，Today 相应选择该项目或全部项目，但项目列表仍保留整个产品集。
