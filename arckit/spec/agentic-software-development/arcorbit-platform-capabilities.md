...
列表保留内容摘要、产品来源、优先级、处理状态和未读提示。详情展示短 ID、完整正文、提交时间、用户标识与联系方式、附件、处理状态、关联待办，以及用户、开发者和系统消息。项目探测到开发者管理会话能力时，详情支持回复文本、上传回复附件、受控查看消息附件，并在打开会话后标记该反馈通知已读；读取或已读回写失败不隐藏已经取得的沟通记录。
