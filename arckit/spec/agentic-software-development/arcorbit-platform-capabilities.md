...
顶部产品集控件在所有主页面保持可用。它同时显示当前工作集名称、产品数量和当前观察范围，并支持在“项目集全部”与工作集内单个产品之间切换；“管理项目集”用于增删工作集成员，范围切换本身不修改工作集。

各页面共用顶部观察范围并保留每条数据的产品归属。Today 项目栏与其他页面（包括 Chat、Thing、Product/Idea）均按顶部观察范围投影；Today 局部项目选择只在该范围内进一步筛选。观察范围统一改变各页的数据投影与对象选择，不改变自动领取资格、项目成员关系、本地绑定、运行中的任务或持久 Codex thread。

Organization Center 保留组织与成员治理能力，项目内容受顶部范围过滤，组织归属明确显示。
...
平台治理入口保留全局组织与成员能力，产品列表受顶部范围控制；项目创建后的组织归属不在 ArcOrbit 中编辑；项目邀请只从项目上下文创建，并保持一次性结果边界。
...
Workshop Feedback SDK、Console 与 Workshop Todo V2 服务共同给出 `/workshop/v2/feedback` 会话、消息、附件、通知、创建、更新、忽略、恢复和转待办请求合约。已忽略反馈必须能够恢复为待处理：V1 以受控更新同时清除 `ignored` 并恢复 `feedback_state=pending`；V2 通过固定 `POST /feedbacks/{id}/restore` 在事务内把当前 `triage_status=ignored` 且未关联待办的同一反馈原子恢复为 `pending`。原反馈 ID、消息、附件与无关 metadata 保持不变，ArcOrbit 只在服务端返回更新后的反馈后显示成功。ArcOrbit 复用 Workshop 登录身份承担开发者侧能力，不使用 SDK 用户侧 API Key 或 Project 107 身份。
...
Today 项目列表以当前产品集成员配置为成员来源，并按顶部查看范围筛选：查看全部时展示全部成员，查看单项目时只展示该成员；不以已有 Today 配置、本地绑定或 Automation participation 筛除项目。产品集为空时显示成员管理引导；成员增删后列表随之更新，仍有效的局部选择保持，失效选择回到全部项目。

Today 点击项目只切换本页项目及对应内容，不修改顶部查看项目范围。顶部主动切换单项目或全部项目时，Today 相应选择该项目或全部项目，项目列表同步展示该查看范围。“全部项目”仅汇总当前顶部范围，不扩大到隐藏成员；成员或范围变化后，无效局部选择回到当前范围内全部项目。

缺少 Today 必要配置的成员仍可点击；首次点击按项目身份初始化所需本机记录，重复点击复用已有记录。初始化失败保留成员和错误重试入口，迟到结果不覆盖较新的选择；初始化不自动绑定目录、执行 Setup 写入、开启 Automation 或创建业务任务。责任对象保留原始产品、任务状态、执行人和来源。

### Organization

Organization Center 以组织 → 成员 → 项目表达治理归属，项目列表受顶部范围控制，并提供个人项目同级范围。
...
开发者可以在详情中调整处理优先级、忽略反馈、恢复为待处理、刷新、删除有权限的反馈或确认转待办。已忽略且未关联待办的反馈显示“恢复为待处理”。V1 与 V2 都只有在服务端确认同一记录为 `pending` 后才更新状态和动作；提交期间锁定恢复动作，失败时保持 ignored、筛选、选择和阅读位置，不做乐观状态改写。已关联待办的反馈展示待办 ID，不再提供重复转待办，优先级改由关联待办管理。删除需要明确确认；所有动作以服务端权限和真实响应为准。
...
4. 用户可以建立、管理包含一个或多个产品的本地工作集，并在任意主页面通过顶部产品集控件快速切换“项目集全部”或单个产品。
5. 所有主页面的产品内容（包括 Chat、Thing、Product/Idea）使用同一个顶部观察范围并保留产品归属，同时不改变全局自动领取资格。
