...
给定 `workshop-todo` 仓库没有 V2 路由、模型或 handler；Workshop Feedback 控制台的 `feedbackV2Client`、条件开关和开发者会话组件共同构成 ArcOrbit 开发者管理能力采用的 V2 客户端合约。该合约足以确定受限 adapter 的请求、响应和页面行为，不要求在实现前另行完成真实环境 API 验证，也不据此宣称给定服务端仓库已经实现 V2。

ArcOrbit 对该合约采用失败关闭：只有项目启用 V2 且受限 adapter 可用时才展示消息、回复附件、通知/已读、专用忽略和原子转待办；路由、认证或权限请求失败时保留反馈工作台的可用事实，隐藏或降级失败能力，并呈现脱敏错误。
...
页面在顶部产品集观察范围内使用左侧反馈列表和右侧详情面板。列表支持按正文、短 ID 和用户信息搜索，按处理状态筛选，并按新旧时间或优先级排序；进入页面、切换范围或筛选导致原选择失效时自动选择首条结果，选择列表项只更新右侧详情。

列表保留内容摘要、产品来源、优先级、处理状态和未读提示。详情展示短 ID、完整正文、提交时间、用户标识与联系方式、附件、处理状态、关联待办，以及用户、开发者和系统消息。项目启用开发者管理 V2 时，详情支持回复文本、上传回复附件、受控查看消息附件，并在打开会话后标记该反馈通知已读；读取或已读回写失败不隐藏已经取得的沟通记录。
...
不同目标环境的能力差异通过项目开关、动作是否可用、状态说明和恢复反馈表达，不在页面标题、列表、详情或提示中暴露协议版本。V2 客户端合约由 Workshop Feedback 控制台前端定义；ArcOrbit 不把真实环境预验证作为显示这些能力的前置条件，但任何失败请求都必须按对应能力失败关闭。
...
3. 用户无需打开反馈控制台即可完成目标项目的开发者反馈日常处理，包括检索、详情阅读、优先级与忽略、消息回复、回复附件、未读处理和转待办；未启用的项目能力按真实响应降级。
...
18. Feedback 支持搜索、处理状态筛选、时间/优先级排序和稳定选择；项目启用开发者管理 V2 时还支持用户/开发者/系统消息、回复附件、未读与已读、专用忽略和原子转待办，失败能力独立降级且不影响已取得的反馈事实。
