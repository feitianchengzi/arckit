...
最终产物类型描述本项目稳定改变行为的对象主要是什么。

Code 类最终产物包括应用、服务、库、脚本、配置、部署产物和运行环境。它们通过代码、运行结果、测试、日志、用户行为和线上状态表达实现事实。

Skill 类最终产物包括 `SKILL.md`、工作流规则、prompt 约束、安装同步配置和其他 Agent 行为约束。它们通过 skill 文件、Agent 执行行为、真实任务试跑结果和分发状态表达实现事实。

不同最终产物类型共享同一套软件研发操作层，但验证方式不同。Code 类项目验证软件是否按预期运行，Skill 类项目验证 Agent 行为是否被稳定改变。
...
当项目的最终产物是 skill 时，评测集用于验收 skill 是否能在真实研发场景中稳定改变 Agent 行为。

当项目的最终产物是 code 时，评测集用于验收软件行为、运行证据和真实用户场景是否满足预期。
