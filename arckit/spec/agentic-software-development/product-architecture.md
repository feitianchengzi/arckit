...
默认 Loop 由同一 Codex Agent 完成；Skill 是该 Agent 原生加载的能力包。`using-arckit` 提供 Agent Loop 方法，`arckit-development-ledger` 独立提供状态协议和 Runtime trusted writeback 接口。Runtime 不固定业务路线、skill、role 或路径，也不为验证、修复或提交创建其它 Agent 身份。
