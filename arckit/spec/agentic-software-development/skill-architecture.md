...
实现型 handoff 至少包含：目标最终产物类型、预期事实依据、影响范围、禁止触碰范围、实现约束、验收口径、需要回查的事实源、需要保留的未决项和验证方式。

验证型 handoff 至少包含：验证目标类型、预期依据、实现证据、执行结果、偏差、剩余风险、建议修复入口和是否需要更新评测材料。

实施时，过程型 skill 输出 handoff；结果型 skill 接收 handoff 后只写入稳定事实；外部 Code 或 Skill 实现能力接收 implementation handoff；验证 skill 接收 expected-vs-actual handoff。没有达到接收条件的信息进入 `arckit-pending` 或继续停留在评测材料中。
...
Code 类实现交接以 `implementation_handoff` 为边界。Arckit 在交给普通代码工作流或 `arckit-code` 类外部 coding skills 前，说明本次实现依据哪些 spec、interaction、visual、tech 或治理事实，哪些内容只是候选判断，哪些文件或模块是允许修改范围，验收口径是什么，验证结果应回到哪个质量 skill。
...
Skill 类实现交接以 `skill_implementation_guard` 为边界。Arckit 在交给 ArcForge 类外部 skill 生命周期能力前，说明目标 skill 的产品责任、路由状态、description 暴露边界、输入输出契约、引用资料范围、真实场景验收样本和安装同步影响。外部能力完成实现后，Arckit 只验收这些操作层关系是否成立，不把外部工具能力描述为 Arckit 内部实现能力。
...
该能力预期由 `arckit-evaluation` 或等价 skill 承载。它维护 `arckit/evaluation/` 的场景索引、Code 类与 Skill 类场景分轨、覆盖矩阵、试跑记录和提升建议；它不直接维护 spec、tech、skill 文件或任务计划。评测结论只有经过确认并满足稳定事实条件后，才交给对应结果型或治理型 skill。
...
4. 定义 Code 类 `implementation_handoff` 和 Skill 类 `skill_implementation_guard`，使外部实现能力能被 Arckit 正确编排和验收。
5. 新增或固化评测场景维护 skill，连接 `arckit/evaluation/`、验证 skill 和真实研发活动样本。
6. 固化治理主链路，使发现事项、迭代归属、owner 判断、执行派发和验收回写各有明确承载者。
7. 强化验证类 skill 对 Code 类和 Skill 类最终产物的不同验收方法。
