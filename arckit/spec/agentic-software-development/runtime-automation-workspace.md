...
ArcOrbit Desktop 是以项目待办和验收反馈为两条独立工作来源的自动化执行工作区。用户在主页面观察当前用户可访问的项目、项目产生的待办、跨项目待办执行队列、验收反馈队列、活动 Runtime、人工事项与完成证据；Chat 仅在人工审查执行过程、提交验收问题或 Runtime 明确需要人工输入时按需出现。
...
验收反馈是独立于远端待办的工作对象。用户在已完成或已验收待办的结果审查中提交非空问题后，系统先持久化一个稳定反馈标识、反馈原文、来源项目与待办、来源完成记录、来源 Run 与 Case、原待办的 Codex thread 和执行会话引用，再把它加入验收反馈队列。提交反馈不改变来源待办的 completed 或 accepted 状态，不复制待办，也不把反馈伪装成 pending 任务。

每个验收反馈项独立保存 `queued`、`running`、`awaiting_human`、`blocked`、`resolved` 或 `cancelled` 状态、当前进展、当前 Run、新建 Case、证据与结果。反馈项通过新 Case 保存人工发现的问题、解决方案、实现和验证；来源待办原有 Run、commit、完成记录和 closed Case 保持不可变。多个反馈项可以关联同一来源待办，并按稳定标识分别展示和推进。

验收反馈队列与待办自动化队列分别持久化、分别计数、分别排序。验收反馈队列按可执行时间、创建时间和反馈标识形成稳定顺序；来源待办状态、优先级或待办队列位置不参与反馈队列排序。两条队列共享一个显式执行仲裁层：当前执行不被新反馈抢占；执行器空闲时，从两条队列各自的 ready 队首中按最早 ready 时间和稳定标识选择一项，并取得工作区与 thread 执行租约。等待执行只改变反馈项进展，不改变来源待办状态。

### 活动任务

工作区在任一时刻最多拥有一个活动执行。活动执行可以来自待办队列或验收反馈队列，并关联一个项目、本地工作区、持久 Codex thread、执行会话和 canonical Case；Runtime run 或交互式 Codex CLI 只是该 Case 当前的执行入口。远端状态不确定、存在多个进行中任务、执行租约冲突、人工 Gate 或完成写回失败时，系统不启动下一项。
...
历史完成 Run 和 closed Case 保持只读，但已完成和已验收待办的会话允许提交新的验收问题。反馈原文追加为该待办会话中的用户消息，新的反馈 Run 复用同一持久 Codex thread，并以 fresh Project/Case State 创建新的关联 Case；它不修改既有完成消息或旧 Case。当前人工介入只加载活动执行的会话；项目级默认 Chat 或其他人工对话不作为自动待办或验收反馈的隐式消息容器。
...
Workbench 保持窗口壳、左右信息栏和底部输入区稳定，只有中间消息列表垂直滚动。用户位于列表底部时新消息自动跟随；用户向上审查历史后保持阅读位置，并提供返回最新消息的明确动作。普通只读审查不提供运行控制输入；completed/accepted 结果审查可以提供独立验收问题 Composer，它只创建反馈项，不修改旧运行。两种模式使用相同的固定壳层和消息滚动边界。
...
验收反馈状态不属于任务服务器七状态，也不反向驱动来源待办状态。来源待办从 completed 进入 accepted 后，既有验收反馈继续保留并按自身状态推进；新增验收反馈同样只创建反馈项和新 Case。
...
自动化开启、没有未收束活动执行且事实可确认时，执行仲裁器分别读取待办队列和验收反馈队列的 ready 队首，确定性选择一项。待办项先通过服务器条件式领取再启动；验收反馈项不改写来源待办状态，在取得工作区与 thread 租约后直接从其持久记录启动。
...
待评审任务可以在权限与服务器版本均有效时确认为待处理。已完成任务可以在审查结果与证据后标记为已验收。已取消任务的业务事实保持只读；已完成和已验收任务不再修改旧执行结果，但详情面板持续展示全部关联验收反馈及进展，并允许提交新的验收问题。验收问题的后续状态只写入独立反馈项和其新 Case。
...
主页面展示项目列表、七种任务状态入口、同步状态、Runtime 健康、自动化开关、人工事项、当前运行、普通待办队列、验收反馈队列、项目源摘要和最近完成结果。两个队列拥有独立数量、ready/blocked/running 分布和队首摘要，不合并成一个“待处理”数字。
...
- 已完成或已验收待办能够提交非空验收问题；系统持久化独立反馈项，不改变或复制来源待办，不改写旧 Run、commit、完成记录或 closed Case。
- 每个验收反馈项具有稳定标识、独立状态、进展、Run 和新 Case，并复用来源待办的 transcript session 与持久 Agent thread；问题、解决方案和验证可从该 Case 恢复。
- 自动化总览分别展示普通待办队列与验收反馈队列；已完成和已验收任务的详情面板展示全部关联反馈项及其当前进展。
- 两条队列分别排序并通过显式执行租约确定性仲裁；等待中的反馈不会进入待办队列或改变来源待办状态，也不会抢占当前执行。
