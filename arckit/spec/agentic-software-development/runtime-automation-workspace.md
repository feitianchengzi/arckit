...
验收问题队列与待办自动化队列分别持久化、分别计数、分别排序。验收问题队列按可执行时间、创建时间和问题标识形成稳定顺序；来源待办状态、优先级或待办队列位置不参与问题队列排序。两条队列共享一个显式执行仲裁层：当前执行不被新问题抢占；执行器空闲时，从两条队列各自的 ready 队首中按最早 ready 时间和稳定标识选择一项，并取得工作区与 thread 执行租约。等待执行只改变问题项进展，不改变来源待办状态。

### 活动任务

工作区在任一时刻最多拥有一个活动执行。活动执行可以来自待办队列或验收问题队列，并关联一个项目、本地工作区、持久 Codex thread、执行会话和 canonical Case；Runtime run 或交互式 Codex CLI 只是该 Case 当前的执行入口。远端状态不确定、存在多个进行中任务、执行租约冲突、人工 Gate 或完成写回失败时，系统不启动下一项。
...
初始同步完成后，参与自动化的项目和当前活动任务所属项目进入实时订阅。Workshop 为每个项目变更保存单调递增的持久事件标识；实时连接只负责通知变化，Workshop REST 任务结果仍是项目、执行人、任务状态和版本的事实源。ArcOrbit 不直接把 WebSocket payload 当作可领取任务，也不依据事件到达顺序推断最终状态。

ArcOrbit 根据 `system.connected` 握手确定订阅模式。支持事件协议的 Workshop 返回 `schema_version` 和 `latest_event_id`，ArcOrbit 为该项目持久化最近确认消费的事件游标；连接建立或恢复后，从已确认游标开始按序补取持久事件，补取期间到达的实时事件按标识去重和排序。旧 Workshop 不返回这两个字段时进入 `legacy` 兼容模式：不读取、不写入也不推进事件游标，每次连接建立都先执行一次项目级 REST 当前态刷新，此后每个无 ID 通知只作为项目失效信号。服务端升级后，下一次握手自动切换为可恢复模式，不需要迁移旧通知或补历史消息。

两种模式下，事件都只使对应项目快照失效，系统在 300 毫秒合并窗口后按项目和当前执行人重新读取任务，再更新队列。项目、成员或权限事件会触发项目范围刷新、订阅重算和服务端权限复核。ArcOrbit 不要求离线期间保留通知：现代服务通过游标恢复连续性，旧服务通过重连后的当前态刷新恢复正确状态。

全量项目与任务对账每十五分钟执行一次，并在应用启动、系统从休眠恢复和网络恢复时立即执行；旧服务连接重新建立时还立即执行项目级当前态刷新。实时连接断开时保留已有快照并自动重连，不启动分钟级降级轮询。Automation 页面持续提供可见的“立即同步”入口；用户怀疑数据滞后时可以主动触发一次全局同步、订阅重算和执行仲裁。服务器声明游标过期或无法连续补取时，系统丢弃该项目旧增量假设，完成一次项目级 REST 全量刷新并从服务器当前游标重新建立连续性。

远端事实刷新、Runtime/Case 对账和自动领取是三个独立动作。事件刷新或定时对账可以更新快照并请求执行仲裁，但不能直接启动 Runtime、清除恢复项或解除人工 Gate；执行仲裁仍要求自动化总闸、项目参与、本地绑定、任务版本、执行租约和当前活动执行全部满足既有条件。`awaiting_human` 在启动恢复、实时事件、游标补取、项目刷新和全量对账期间保持关闭式保护，只有用户通过当前人工事项提交显式介入输入后才允许恢复同一任务。

项目列表拉取失败时，系统无法确认用户的完整执行范围，因此冻结所有新任务领取。单项目任务拉取失败时，系统排除该项目的新任务领取，其他已确认项目仍可参与队列。

后台同步保留现有内容，并分别展示实时连接状态、订阅模式、现代模式的最近成功事件游标、最近项目刷新时间和最近全量对账时间。同步失败不清空已有列表，也不把本地快照视为服务器最新事实；界面提示用户可立即同步。连接处于补取、重连或异常状态时，受影响项目的新任务领取等待事实重新连续；既有人工事项和本地完成检查点不回退。

活动 Runtime 期间持续核对远端任务版本和执行人。任务被改派给其他用户、外部取消、阻塞、转移项目或撤销权限时，系统请求 Runtime 在安全停止点收束，并在事实重新一致前冻结队列；领取和完成写回前也重新确认执行人仍是当前用户。

## 自动领取与串行执行

自动化开启、没有未收束活动执行且事实可确认时，执行仲裁器分别读取待办队列和验收问题队列的 ready 队首，确定性选择一项。待办项先通过服务器条件式领取再启动；验收问题项不改写来源待办状态，在取得工作区与 thread 租约后直接从其持久记录启动。

执行仲裁不能直接信任排队时的本地快照。每次领取前先从 Workshop 重新读取该任务，确认它仍为当前用户负责、仍处于待处理且版本与候选一致，再基于该最新服务器版本条件式提交“待处理 → 进行中”。服务器确认成功后，系统持久化任务、项目、本地工作区和待启动 run 的关联，再启动 Runtime。
...
Recovery Center 统一承接领取冲突、Runtime 启动失败、安全停止、活动任务外部变化、多个进行中任务、任务项目归属异常、权限异常和任务源会话失效。对已建立持久 Agent thread 的 Runtime 失败，用户可直接补充说明并在同一对话继续，不必先把任务标记为阻塞。
...
主页面展示项目列表、七种任务状态入口、同步状态、Runtime 健康、自动化开关、人工事项、当前运行、普通待办队列、验收问题队列、项目源摘要和最近完成结果。两个队列拥有独立数量、ready/blocked/running 分布和队首摘要，不合并成一个“待处理”数字。
...
- 两条队列分别排序并通过显式执行租约确定性仲裁；等待中的验收问题不会进入待办队列或改变来源待办状态，也不会抢占当前执行。
...
- 新 Workshop 项目变更通过持久事件游标及时唤醒对应项目刷新；应用断线期间的事件能够在重连后按序补取、去重并恢复连续性，游标过期时执行项目级全量恢复。
- 旧 Workshop 的无 ID 通知能够在线触发项目刷新；连接与重连不读写游标，重连后直接刷新当前态，不要求补历史通知。
- 系统不执行每分钟全量扫描；每十五分钟、应用启动和休眠/网络恢复时执行事实对账，旧服务重连后执行项目刷新，Automation 页面持续提供“立即同步”。
