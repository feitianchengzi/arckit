...
当前用户由任务服务器认证会话确定。工作区只展示当前用户可访问的项目及其任务。认证失效时，系统保留最近一次成功同步的只读快照，但停止领取任务和提交状态变更。
...
待办始终归属于一个有效项目，并保留服务器任务标识、版本、状态、优先级和进入当前状态的时间。缺少有效项目归属、无访问权限或无法确认服务器版本的待办不进入自动执行队列。

### 自动化队列

自动化队列由所有参与自动化且具备本地工作区绑定的项目产生。队列只包含服务器状态为待处理且当前版本已确认的任务。
...
应用启动时依次恢复认证会话、当前用户项目、本地工作区绑定、项目自动化参与状态、各项目待办和本地 Runtime 状态。

系统先拉取当前用户项目，再按项目拉取任务。任务同步结果保留项目归属和服务器版本，项目级状态计数由任务快照派生。
...
活动 Runtime 期间持续核对远端任务版本。任务被外部取消、阻塞、转移项目或撤销权限时，系统请求 Runtime 在安全停止点收束，并在事实重新一致前冻结队列。
...
用户提交处理结果后，输入被锁定，直到 Controller 接受该输入或返回可解释错误。Controller 接受后，Runtime 恢复当前任务，界面返回自动化主页面。
...
每个恢复状态展示服务器事实、本地事实、差异、已保留证据、当前责任方和允许动作。恢复动作完成后，系统重新同步服务器并从原观察范围返回 Command Center 或 Task Browser。
