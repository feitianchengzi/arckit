...
- Chat 中央消息与 Composer 保持最大 760px 阅读宽度；760px 及以下会话列表为可收起抽屉，选择后关闭，Esc 恢复触发按钮焦点，页面切换不丢失会话草稿。
- Chat 右栏直接按 Product Workspace 分组展示当前用户的本地会话，并支持新建、选择、重命名和删除。项目组按组内最新活动时间排序，组内会话按 `updated_at` 倒序排列。
