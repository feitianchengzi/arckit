...
`agentic-software-development/arcorbit-work-management.md` 从平台能力规格拆出 Work 的完整日常待办边界，定义多维查询、父子任务树、详情与评论附件、字段维护、受控状态动作和替代 Todo 网页端的验收口径。它消费 `arcorbit-platform-capabilities.md` 的 Product Workspace 与顶部产品范围，同时保持 `runtime-automation-workspace.md` 的 Automation、验收问题和人工 Gate 边界；对应页面事实位于 `arckit/interaction/task-browser/`，技术方案位于 `arckit/tech/arcorbit/platform-composition-solution.md`。

`agentic-software-development/arcorbit-planned-workspaces.md` 在已实现平台核心外定义 Chat、Idea、Release、Operations 和 Engineering 的团队计划展示面，并把主导航组织为 Personal、Product Lifecycle 和 Organization 三组。Engineering 用 Profile Library、State Model、Capability Mapping、Lifecycle Mapping、变更预览和 Apply 确认表达可管理 Domain Profile，明确排除 entry skills，并说明不同领域 Profile 复用同一 Loop Kernel 与产品流程；它消费现有 Product Workspace、Task、Feedback、Run、ledger、代码/配置/测试/运行证据和 release workflow 事实，但不因此建立新的服务端、Runtime、Profile persistence、发布、监控或市场平台合约。对应交互事实位于 `arckit/interaction/chat-workspace/`、`idea-workspace/`、`release-workspace/`、`operations-workspace/` 和 `engineering-profile/`。
