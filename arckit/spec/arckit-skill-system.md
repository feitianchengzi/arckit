...
- 把稳定的规划、范围、决策、任务和路线图影响交回 `arckit-project-governance-workflow`。
...
`arckit-project-governance-workflow` 是项目控制层，负责 Backlog、Goal、scope、Iteration、Task、Review、Decision、Roadmap 和项目文档结构。
...
`arckit-decision-framework` 是横向决策方法 skill。它既可作为独立决策工作流，也可在其他主工作流的关键判断点提供辅助方法。

`arckit-decision-framework` 支持以下判断场景：
...
当 `arckit-decision-framework` 作为辅助方法使用时，它不创建独立决策文件，除非用户明确要求。它返回小型决策片段给主工作流，由主工作流写入自己的文档、记录、任务或规格。

主产物 skill 不硬编码 `arckit-decision-framework`。主产物 skill 只描述自身关键判断点，agent 根据 `arckit-decision-framework` 的 description 在相关时机识别它可作为辅助方法。
...
| `arckit-project-governance-workflow` | Goals、scope、iterations、tasks、reviews、decisions、roadmap 和项目文档结构的治理控制层。 |
...
- `arckit-decision-framework`、`arckit-idea`、`arckit-idea-explore` 的 description 需要承担触发和边界责任，避免正文补救式说明。

## 9. 后续规格关注点

后续可继续完善：

- 明确 Arckit 只定义自身 skill 产物的创建、验证、审查、稳定化和项目内治理规则；外部安装、同步或跨仓库分发工具不属于 Arckit 产品范围。
- 继续 forward-test `using-arckit`，覆盖一个主 skill 与一个横向辅助方法协作的混合场景。
- 当多 skill workflow 的 token 成本成为实际问题时，把 `arckit-decision-framework` 的长方法细节拆到 references 中。
