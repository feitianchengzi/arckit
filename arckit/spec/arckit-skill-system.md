...
Arckit 使用 `using-arckit` 作为入口 skill。该 skill 独立位于 `entry/skills/using-arckit/`，负责把用户请求编译为连续项目状态、`development_case_record` 和本轮 `workflow_frame`，路由 `arckit-development-ledger` 维护研发状态账本，选择最小必要 skill 集，并组织当前轮执行顺序。
...
| `arckit-turn-adaptation` | 后续消息对当前 frame、事实路由、停止条件或 workflow memory 判断的变更控制。 |
