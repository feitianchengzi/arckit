...
Arckit 是围绕本仓库维护的软件项目协作技能包。它的工程目标是指导 agent 辅助人类完成软件项目开发，让 agent 能理解任务处境、维护项目事实、形成可执行交接、定位实现问题，并把过程经验沉淀为可复用工作方式。

当前能力体系覆盖：

- 入口编排和后续消息适配。
- 研发事项 case record、完成度审计和下一轮缺口判断。
- 原始输入、未决上下文和 workflow memory。
- 决策、草案、设计探索、架构决策和领域建模。
- 产品、交互、视觉和技术事实源维护。
- 基于证据的 debug 诊断。

技术栈专属编码实践由独立的代码实践仓库承载，例如 `arckit-code`。项目治理、代码审查、评测、发布出包、运行运维、Workshop Desktop、多角色编排和长期想法管理进入 Arckit 时，先沉淀为决策依据、定义事实、诊断证据、pending 交接项或 external adapter handoff。

## 2. 入口路由

Arckit 使用 `using-arckit` 作为入口 skill。该 skill 独立位于 `entry/skills/using-arckit/`，负责把用户请求编译为 `development_case_record`，维护 `arckit/cases` 中的研发事项状态，选择最小必要 skill 集，并组织当前轮执行顺序。

`using-arckit` 支持以下行为：

- 根据用户目标识别当前阶段、期望产物和当前最关键未满足结构。
- 创建或更新 `arckit/cases/active/` 下的 development case record，并在每轮结束前完成 completion audit。
- 调用 `arckit-workflow-memory` 做 workflow resolution。
- 选择当前能力体系中最小相关 skill 集，保持 agent 的上下文和行动聚焦。
- 把稳定产品、交互、视觉和技术事实交给对应结果型 skill。
- 把需要后续确认、外部执行或跨工具承接的事项交给 `arckit-pending`。
- 把首轮之后的补充、纠错、换目标、事实纠正和暂停交给 `arckit-turn-adaptation`。
...
| `arckit-turn-adaptation` | 首轮目标之后的补充、纠错、换目标、事实纠正、澄清回答和暂停分类。 |
...
项目级研发事项记录位于 `arckit/cases/`，由 `using-arckit` 和 `tools/arckit-case/arckit-case.mjs` 维护。它不是 skill，但为入口编排提供跨轮次状态承载。
...
`using-arckit` 将这些阶段拆成当前可处理的决策、定义、技术、诊断或未决记录，并输出 `external_adapter_handoff` 或写入 `arckit-pending`，便于人类或外部工具继续推进。
