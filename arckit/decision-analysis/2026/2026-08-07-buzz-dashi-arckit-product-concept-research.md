...
前文采用统一框架客观解释三个项目各自成立的原因。本附录进一步做一个不同性质的判断：如果不追求面面俱到，而是必须选择一个最符合长期 AI 发展的核心方向，选择什么？

结论是：

> 在三个项目中，选择 Arckit 的 `state-driven loop`。评价的是核心方向，而不是当前产品完成度；排序为 Arckit、Buzz、Dashi。

### A.1 判断起点：智能的本质是什么

从第一性原理看，智能不是产生内容、执行指令或留下活动记录，而是在变化和不确定的环境中，持续判断现实、选择行动，并根据行动结果修正后续行为：
...
这就是最基本的 state-driven loop。Task、Event、Skill 和 Identity 都具有重要价值，但分别回答的是不同的次级问题：

- Task：准备做什么，以及由谁承担。
- Event：发生了什么，以及由谁产生。
- Skill：可以怎样做。
- Identity：谁在行动，以及拥有什么关系和权限。
- State-driven loop：为什么行动、行动是否有效，以及下一步是什么。

未来 AI 的核心不只是更高效地产生行为，而是更可靠地让现实向目标推进。

### A.2 删除测试：哪个核心不可替代

判断一个概念是否足够根本，可以暂时删除它，观察自主 AI 是否仍然成立。

#### 删除 Task lifecycle

没有任务板，AI 仍然可以观察当前状态、发现问题、形成计划、执行、验证并继续。随着 Agent 能力提升，Task 还可以由 AI 动态生成，只作为面向人类注意力和责任分配的工作投影。

因此，Dashi 的 Task lifecycle 是非常现实且高价值的当前产品入口，但不是持续智能不可删除的控制核心。模型越强，预先由人拆好任务的重要性可能越低。

#### 删除 Signed event 和持续身份

没有 Buzz 的身份与事件协议，单个 Agent 仍然可以完成状态闭环，但多人、多 Agent 的协作会失去可信来源、共同历史和责任边界。

因此 Buzz 比单纯任务系统更接近长期基础设施。它回答的是“智能行动者如何进入可信协作网络”，但事件被正确签名并不证明事件所代表的行动让现实变得正确。一个组织可以拥有来源完整、全程可追溯，却仍然错误的决策历史。

#### 删除 State-driven loop

如果 AI 不维护当前状态、不识别差距、不验证结果，它仍然可以响应 Prompt、领取 Task、产生 Event 和调用 Skill，却无法判断软件、业务或现实是否真的改变。

模型能力越强，这个缺口越危险：系统可以更快地产生更多行为，却没有可靠机制判断行为是否有效。因此删除 state-driven loop 会直接破坏持续自主行动本身。
...
第一阶段关心模型能否生成有用内容。第二阶段关心 Agent 能否使用工具、修改代码和完成任务。长期阶段关心的是：Agent 能否在没有人逐步指挥的情况下，持续维护对现实的理解，发现差距，组织行动，验证结果，并在现实反证出现时修正自己。

当生成、工具调用和局部执行逐渐商品化，真正稀缺的会变成：
...
这些问题都直接落在 state-driven loop 上。

### A.4 为什么不再提出一个新的综合核心

Evidence、Goal、Uncertainty、Learning 和 Human Governance 都可以成为未来系统的重要能力，但没有必要用它们重新包装一个同时包含三者的新核心：

- Evidence 是接受状态变化的依据。
- Goal 定义目标状态和价值方向。
- Uncertainty 是状态表达中尚未确定、需要继续验证的部分。
- Learning 是根据多轮状态转移改进能力选择。
- Human Governance 决定哪些状态变化必须经过人的判断或授权。

它们都可以从 state-driven loop 内部继续生发。Buzz 的身份与事件机制可以成为可信协作基础设施，Dashi 的任务体验可以成为面向人的工作表面，但它们不应取得软件演进的最终解释权。

最终解释权应属于：

> 现实现在是什么，以及 AI 是否真的让它发生了可信的改变。

### A.5 选择核心，不等于无条件认可当前实现

选择 Arckit 的方向，不代表当前 Arckit 已经是最好的产品实现。它仍然面对几项决定成败的风险：

- 软件状态建模和持续维护的成本可能过高。
- 过度确定性的协议可能压制真实研发中的模糊探索。
- 如果 Case、facet、transition 和 Evidence 等内部术语直接暴露给用户，产品可能正确但难用。
- 如果状态维护成本大于它带来的判断价值，平台会退化成沉重的研发档案系统。
- 如果不能通过真实交付证明它比“Task + Agent”产生更可靠的结果，理论优势就没有转化成产品价值。

这些是产品化与实现风险，不改变核心方向上的选择。Arckit 需要做到的是内部状态语义严格，外部产品体验自然，并持续用真实研发结果证明状态驱动闭环的价值。

### A.6 最终选择

如果只能押一个长期方向，不选择更好的 AI Taskboard，也不首先选择 Agent 协作社交网络，而选择：

> 让 AI 围绕真实世界状态持续行动，并且只有在证据足够时才接受变化。

在三个项目中，这就是 Arckit 的 `state-driven loop`。Buzz 和 Dashi 揭示的能力仍然重要，但更适合作为协作基础设施和工作投影，服务于这一核心，而不是取代它。
