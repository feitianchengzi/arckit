...
刷新采用 single-flight；读取期间的内容失效保留到下一轮，用户显式刷新消费已有排队通知。详情请求失败不推进复用标识，任务切换使用 selection epoch 拒绝旧详情；账号范围切换清除旧详情，已移除事情不保留原详情。没有首次加载状态时不先重绘一遍旧内容，未变化详情复用已渲染内容。snapshot 携带轻量同步健康，主壳据此更新标题栏而不读取旧页面全量数据。
