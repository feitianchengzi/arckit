...
删除事件保留实体标识和删除时间。成员新增、删除等授权事件把目标用户写入 `subject_id`。事件固定保留三十天；清理只删除早于保留边界的事件，不改变仍存在事件的标识。事件 ID 是全局游标，因此服务使用全局事件日志的最早保留 ID 判断游标是否仍可连续恢复，并在项目补取响应中返回该项目的可见边界。
...
每个 Workshop 实例持有独立 PostgreSQL LISTEN 连接。收到事件 ID 后，实例从 `project_events` 读取完整记录并广播到本实例对应项目房间。通知只承担低延迟唤醒；监听连接重建时，实例从自己的最近分发游标补查事件表，因此 LISTEN 短暂中断不会使仍连接于该实例的客户端永久漏失事件。
...
项目补取接口为 `GET /workshop/v1/user/projects/:id/events?after_id=<cursor>&limit=<n>`。它只允许当前项目成员访问，按事件 ID 升序返回不大于服务器限制的事件页、`next_after_id`、`latest_event_id`、`earliest_event_id` 和 `has_more`。`after_id` 早于当前保留边界时返回稳定的 `EVENT_CURSOR_EXPIRED`，客户端必须执行项目级 REST 全量刷新，而不能假装已经连续消费。
...
Website 不把事件 payload 直接写入 Query cache。连接重建会先完成补取或全量失效，再恢复 `connected` 状态，避免断线期间变化永久停留在旧页面。
...
连接成功后 Adapter 先检查 `system.connected`。握手包含受支持的 `schema_version` 与安全整数 `latest_event_id` 时进入 `resumable`：读取边界并补取 `(confirmed_id, latest_event_id]`，同时缓冲新到达的实时事件；补取完成后把缓冲与历史事件按 ID 合并、去重并顺序交付。只有对应项目的 REST 刷新成功后才推进持久 confirmed cursor；WebSocket 收到事件本身不能推进 checkpoint。
...
- 补取接口覆盖顺序、分页、重复请求、无权限和游标过期。
- Website 测试覆盖断线期间事件、重连补取、重复/乱序事件和过期游标全量失效。
- ArcOrbit 测试覆盖订阅调整、退避、token 代际、项目去抖刷新、现代 cursor checkpoint、旧握手识别、无 ID 通知刷新以及旧模式不读写 cursor。
