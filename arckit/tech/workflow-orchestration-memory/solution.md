...
### 2.6.1 Loop Handoff Gate

Loop Handoff Gate 在 `before_final` 阶段运行，由入口编排和 development ledger 共同承载。它生成 `completion_audit.loop_handoff`，供人类手动触发下一轮或未来自动桥调度。

Gate 的输入包括最终 Workflow Frame、artifact impact scan、case audit、project loop control、验证结果、pending handoffs、workflow memory closeout 和用户确认点。

Gate 的输出至少包含：

- `status`：continue、done、needs_human、blocked 或 deferred。
- `next_responsibility`：agent、human、external 或 none。
- `agent_continuation_available`。
...
Gate 先判断下一步职责归属，再判断触发方式。`manual_bridge` 只表示下一步本应由 Agent 继续，但当前缺少自动续轮调度器，需要人手动触发；它不表示需要人类决策。需要人类判断、授权、审美、商业取舍、风险接受或发布责任时，Gate 输出 `next_responsibility=human` 和 `human_decision_required=true`。等待 CI、云控制台、第三方审批、本地人工编译结果或其他系统外结果时，Gate 输出 `next_responsibility=external`。

自动桥 controller 只消费满足以下条件的 handoff：
...
如果条件不满足，controller 停止自动续轮并把 `human_gate`、外部等待条件或完成状态展示给人类或上层平台。
...
13. `Loop Handoff Gate` 生成 `completion_audit.loop_handoff`，把本轮 closeout 转化为下一轮可消费的职责和触发契约。

该流程允许入口 skill 从“关键词式 skill 选择”升级为“首轮场景识别和工作流编排器”，同时避免它膨胀为执行中所有消息和记忆判断的总控。

## 6.1 Loop Handoff Schema

Loop Handoff 存在于 development case record 的 `completion_audit.loop_handoff` 中。项目级摘要可投影到 `project_state_record.loop_control`。
...
`next_prompt` 是面向人类手动触发下一轮的自然语言桥接文本。`agent_instruction` 是面向自动桥或多 Agent 平台的结构化输入。两者表达同一个下一步，不应产生不同事实。

`progress_guard` 用于防止自动桥在没有状态变化时无限续轮。自动桥应比较 `expected_state_change` 和下一轮实际写回的 project/case delta；连续无进展达到 `no_progress_limit` 或超过 `max_auto_rounds` 时停止自动续轮并输出阻塞状态。
