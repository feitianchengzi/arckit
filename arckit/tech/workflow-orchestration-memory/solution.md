...
工作流编排与自然沉淀方案为 Arckit 提供 runtime workflow compiler、workflow memory overlay 和 artifact impact router。该层位于 `using-arckit` 与专门 skill 之间，负责把用户任务转化为可执行的 skill 编排框架，并把有学习价值的任务轨迹沉淀为 signals、candidates 和经用户确认的 workflow patches。
...
Workflow Composer 不是复用一份固定 workflow 的执行器，而是 runtime compiler。它使用 Arckit 默认能力地图、当前 situation model、用户显式要求和 workflow memory overlay 编译本轮 Workflow Frame。
...
9. 根据执行反馈调整当前 Workflow Frame。
10. 执行 artifact impact scan，并把稳定事实、未决问题、治理影响和流程学习路由到不同 skill。
11. 任务结束、阻塞或失败时调用 `arckit-workflow-memory` 做 signal decision。
12. 若 decision 是 `write_signal`，且 memory root 不存在但可写，直接初始化并写入 signal；如果不可写，输出 bootstrap/signal pending write；不能只口头报告未写。
13. 若 decision 是 `write_signal`，把本轮 signal 加入 pending signal buffer。
14. 若 decision 是 `update_candidate_only`，只更新 candidate 的 `match_count`、`success_count`、`failure_count`、`last_matched_at`、`last_outcome` 或 `last_match_summary`。
15. 第二条相似 signal 后维护 candidate patch；若 signal 文件尚未落盘或 memory root 不存在，输出 candidate pending write。
16. 每次写入或更新 signal、candidate、accepted workflow patch 后同步对应 `INDEX.md`；若索引不同步，输出 index pending/blocked 状态。
17. 经用户确认后写入 accepted workflow patch。
...
2. 用户自然对话中的流程偏好、纠正、失败、阻塞或新增模式作为 signal 或 candidate 证据。
