...
| `task-browser/` | ✅ | 在 Work 的单行控制轨中组合本地查询，并用 Work-owned 同步和任务树/Inspector 完成详情、评论附件、产品限定维护、执行资格引导及三入口七状态修改；Automation 只消费确认结果 | 本地查询、Work Sync、单行控制轨与窄窗口收敛、多维筛选、任务树、完整详情、评论附件、三入口七状态 Picker、待评审/执行人/项目连接引导、标签生命周期、运行/验收快捷动作、外部状态恢复、空态与冲突 |
