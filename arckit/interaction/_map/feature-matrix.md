...
| `automation-workspace/` | ✅ | 登录后只消费 Work 发布的本地待办状态，以本地 workspace lane 串行仲裁普通待办与验收问题，并在最多 3 条独立 lane 间并行；候选不可领取时解释首要原因并原位修复 | Work 同步健康摘要、资格原因引导、双队列总览、活动执行选择、槽位容量、lane 串行、跨 workspace 并行、问题等待/运行/待人工/阻塞、external dependency 人工介入与同 thread 恢复、项目范围切换、CLI 接管、人工介入、外部等待、执行停止、完成续接、Work 动作失败、领取冲突与用量诊断 |
