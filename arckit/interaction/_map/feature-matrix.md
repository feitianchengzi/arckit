...
| `chat-workspace/` | ✅ | 按 Product Workspace 分组浏览最近与历史会话，在首条消息前显式确认项目归属，并从 Composer 快速调整当前会话 Model/Level | 项目分组、每组最近 10 条、历史展开/收起、新对话项目切换、会话级 Model/Level、turn 配置固定、流式消息、工具/权限、停止、中断/失败恢复、删除确认、工作区阻塞与绑定 Sheet |
