...
用户在 Automation Command Center 中先掌握当前用户可访问的项目，再分别查看普通待办队列、验收反馈队列、自动化吞吐、当前运行和需要人工判断的事项。系统按项目拉取待办形成跨项目待办队列，并从已完成或已验收待办的结果审查接收独立验收反馈；Chat 仅在审查执行过程、提交验收问题或处理人工中断时进入独立的 Intervention Workbench。
...
10. 用户从已完成或已验收任务的右侧详情面板查看全部验收问题、状态和最近进展，选择某一项进入其同一待办对话，或提交新的非空验收问题。
11. Command Center 用独立区块展示普通待办队列与验收反馈队列；用户可以分别查看 ready、running、awaiting human 和 blocked 数量，不会把反馈问题计入待处理任务。
...
4. Command Center 展示项目列表、当前项目范围、自动化健康、需人工处理数量、活动执行、普通待办队列、验收反馈队列和最近完成结果。
...
12. 需要人工时，系统暂停当前推进并创建 attention item；用户在 Intervention Workbench 提交处理结果后，系统恢复当前任务并返回 Command Center。
...
- 旧完成结果保持只读；已完成或已验收待办的结果审查提供独立“提交验收问题”输入。提交后新增反馈项和用户消息，不修改旧 Run、旧 Case 或来源待办状态。
- 验收反馈队列和普通待办队列在信息架构、计数、筛选和排序上彼此独立；执行器是否空闲只影响反馈项何时从 queued 进入 running，不改变它属于哪条队列。
- 一个验收反馈项显示稳定标识、来源待办、问题摘要、状态、当前 Case/Run、最近进展和阻塞/人工责任。多个反馈项按创建时间分别展示，不合并为待办的一条备注。
...
- 普通只读审查隐藏运行控制输入；人工处理模式显示输入区与“提交并恢复自动化”。completed/accepted 结果审查可以显示独立验收问题 Composer，它不等同于介入旧运行。
...
`验收反馈 queued → running → awaiting_human | blocked | resolved | cancelled`

两条状态流仅通过来源引用、同一待办会话和执行租约关联；验收反馈状态不回写来源待办状态。
...
- `default.html` 投影应用壳、项目列表与范围切换、Command Center 待命、自动领取配置缺口、启动同步、自动执行中、交互式 Codex CLI 接管、人工介入入口、完成续接、空队列和异常恢复。
- `default.html` 还投影普通待办与验收反馈双队列总览，以及验收反馈等待执行、执行中和需人工状态。
...
| 独立验收反馈队列 | 存在已提交验收问题 | 双队列摘要、反馈表格、进展徽标 | 查看反馈进展或进入来源对话 |
...
- 普通只读模式不展示运行控制 Composer；“介入当前运行”需要明确操作。completed/accepted 结果审查的验收问题 Composer 独立创建反馈项，不改变旧运行。
