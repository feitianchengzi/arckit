...
- 任务是分别设置当前设备新建 Chat 会话和新启动 Automation Run 所用的 Model 与 Level；主路径为打开账号覆盖层、分别编辑或选取两组候选、保存、原位确认生效范围。
- 进入已登录设置自动获取清单；Chat 与 Automation 各有两个带候选提示的文本输入框。Model 候选展示模型名称，各自的 Level 候选随对应 Model 更新。切换模型不自动修改 Level 值，用户可按候选自行调整。
