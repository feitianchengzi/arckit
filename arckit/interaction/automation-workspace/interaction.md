...
14. 用户通过全局侧栏底部的头像打开原账号页面，查看任务源、会话和 Runtime 设置；Workshop 账户标题显示平台 current-user 的名称。
...
3. 系统按项目拉取待办并保留 `project_id` 归属，再为参与自动化项目和当前活动任务项目建立实时订阅；事件按项目合并后触发 REST 刷新，再计算各项目状态数量与跨项目执行资格。Automation 消费顶部产品集的观察范围，不自行维护第二套项目筛选。
...
6. 系统先重新读取远端任务并确认执行人、状态和候选版本，再基于最新服务器版本条件式提交 `待处理 → 进行中`；领取冲突时刷新队列并选择下一项，不启动 Runtime。
...
13. 实时连接断开时，系统自动重连但不启用分钟级轮询；现代服务重连后补取事件，旧服务重连后直接刷新当前态。页面始终允许用户“立即同步”。
...
- 实时事件、补取、后台对账、应用重启和网络恢复只更新远端事实与连接健康。它们不能把“等待人工”改为“执行中”，也不能替代用户提交介入说明。
- 人工事项存在时，实时链路仍继续确认远端任务是否被取消、改派、阻塞或撤销权限；无冲突更新保持原人工事项，有终止性冲突时进入 Recovery Center，但两种情况都不自动继续 Agent。
...
| 启动同步 | 应用启动、账号切换、手动立即同步 | ProgressView、toolbar | 等待或打开设置 |
...
- Automation 页头持续展示“立即同步”。点击后禁用按钮并显示“同步中…”，通过主进程执行全局当前态同步、订阅重算和一次执行仲裁；成功提示最近同步时间，失败保留现有快照并展示可重试错误。
...
| 实时事件 | 现代模式按项目在 300 毫秒内合并，刷新对应项目任务，成功确认 REST 结果后推进事件游标；旧服务无 ID 通知同样触发项目刷新，但不读写游标 |
| 实时重连 | 现代模式显示连接中和补取状态，补取、去重、项目刷新完成后显示实时；旧服务重连直接刷新当前态并显示“实时兼容连接” |
| 实时不可用 | 保留内容和数据时间，显示连接异常、“可立即同步”与自动重连状态；不启动分钟级降级轮询 |
| 事实对账 | 每十五分钟执行一次；启动、系统唤醒和网络恢复时立即执行，旧服务重连时立即刷新对应项目 |
| 手动立即同步 | 用户点击页头入口后执行全局同步、订阅重算和一次执行仲裁；同步不会解除人工 Gate |
