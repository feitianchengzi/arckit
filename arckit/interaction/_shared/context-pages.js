...
(() => {
 ...
 const stored=C.state;stored.pageObjects??={};stored.account??={tasks:[],offline:false};
 ...
 function records(){
  ...
  return items.filter(p=>C.includes(p.project));
 }
 ...
 function render(){
  const changed=lastScope!==C.scopeKey();const items=records();
  ...
  root.innerHTML=`<div class="gc-page-toolbar"><h1>${esc(heading)}</h1>${page==='Work'?`<button id="gc-new" ${!C.ids().length?'disabled':''}>新建${titles[page]||'事项'}</button>`:''}</div>${content}<p class="gc-note">本地交互原型 · 所有保存与运行状态为模拟，不连接业务服务。</p><a class="gc-note" href="${page==='Organization'?'governance-states.html':'page-states.html'}">查看页面专项状态说明</a>`;
  C.save();
 }
 ...
 C.before(()=>{if(selected)C.remember(page,selected);});C.on(()=>{if(lastScope!==C.scopeKey())render();});render();
})();
