...
- Feedback 使用左侧列表与右侧详情面板。搜索、状态筛选和排序限定列表；进入页面或当前选择不再命中结果时自动选择首条，选择反馈只更新详情，不离开页面。
- 处理动作集中在详情：调整处理优先级、忽略、刷新、按权限删除和转待办。已关联待办时展示待办 ID并移除重复转待办入口，优先级在关联待办中继续管理。
- 项目启用开发者反馈管理能力时，详情在原反馈事实下方展示用户、开发者和系统消息，允许开发者发送文本或附件回复，并显示未读提示；打开会话触发该反馈的已读回写，但已读失败不隐藏消息。
...
右侧详情固定展示所选反馈的短 ID、完整正文、提交时间、产品来源、用户标识、手机号、邮箱、附件、处理状态、优先级和关联待办。项目启用开发者管理 V2 时，详情还展示用户、开发者和系统消息及附件；开发者可以发送文本或附件回复，附件上传与读取均使用服务端签发的受限策略或凭证。未启用项目只展示已有反馈事实，不伪造沟通时间线或回复入口。

反馈列表和页头显示项目范围内的未读数量，含未读更新的反馈显示可辨识提示。开发者打开某条反馈的沟通区后，系统读取消息并请求把该反馈通知标记为已读；消息读取失败显示恢复动作，已读回写失败只保留未读提示，不清空或隐藏已加载消息。

开发者在详情中调整处理优先级、忽略、刷新、按权限删除或触发“转待办”，不能改写用户提交的反馈事实。删除先显示明确确认。项目启用开发者管理 V2 时，忽略使用专用动作，转待办使用返回反馈、任务和关系的服务端原子动作。反馈已经关联待办时，详情展示待办 ID，不再允许重复转待办，也不在反馈中继续调整优先级。
...
- `HUMAN-NOTE-PLATFORM-002`：Workshop Feedback 控制台前端的 V2 client、项目开关和会话组件是开发者管理能力的采用契约，不要求真实环境预验证；ArcOrbit 仍通过受限 adapter、项目开关和逐动作失败关闭控制消息、附件、通知与原子流转。
