...
- Organization Center 直接使用 Workshop Organization、OrganizationMember 和 Project Member。服务端实际校验决定请求结果；直接添加的客户端角色限制不代表服务端具有相同保护。
- 项目创建时确定个人或组织归属；现阶段不提供创建后的组织迁移入口。
- ArcOrbit 仅允许项目 owner/admin 从同组织成员中直接添加项目成员；主进程核对项目角色，现有服务端不新增对应角色校验。项目邀请作为独立加入方式保留。
...
- Organization 的直接加人路径是“组织 → 项目 → 项目成员 → 从组织添加成员 → 搜索并选择一位成员 → 添加 → 服务端确认”。Sheet 固定显示目标组织和项目，新增角色为 member；已有成员不可选。入口按项目 owner/admin 角色显示，组织角色不替代项目角色。
- 直接添加的状态流包含候选加载、选择、空结果、提交、确认成功、结果不明核对和拒绝恢复。只有服务端确认改变成员投影；刷新失败仅重读，过期响应不夺回用户已切换的项目或账户。状态投影见同页 `member-add.html`。
...
- 处理动作集中在详情：调整处理优先级、忽略、恢复为待处理、刷新、按权限删除和转待办。只有处于已忽略且未关联待办的反馈显示“恢复为待处理”。V1 通过兼容 metadata update 原地恢复；V2 通过固定 restore 领域动作原子恢复同一记录。只有服务端确认 `pending` 后才替换状态和动作；失败时保持 ignored、筛选、选择与阅读位置并显示真实错误。已关联待办时展示待办 ID并移除重复转待办入口，优先级在关联待办中继续管理。
- 当前项目的受限 V2 开发者能力探测成功时，详情在原反馈事实下方展示用户、开发者和系统消息，允许开发者发送文本或附件回复，并显示未读提示。首次选择或 fresh notification snapshot 标记当前反馈有未读回复时自动重拉会话；页面级、详情级和沟通记录刷新都先刷新反馈与通知事实，再重拉当前会话。消息成功投影后才回写已读；消息或已读失败保留旧消息、未读提示、草稿、附件、详情滚动位置和可见重试。
...
顶部产品集控件始终显示当前集合名称、产品数量和观察范围。用户可以在“项目集全部”与集合内单个产品之间快速切换，并通过“管理项目集”打开成员管理覆盖层；Idea、Work、Automation、Release、Operations 和 Feedback 共享该选择；Today 展示产品集当前查看范围内的成员，顶部主动切换单向设置其局部选择，Today 内点击不回写顶部，Chat 可选择是否携带该上下文。Organization 与 Engineering 使用组织能力范围，不用产品集名称冒充组织范围。
...
3. 选择项目打开项目详情。事实编辑、本地绑定、Workset 展示范围、Automation participation 是四个独立动作，不能互相隐式修改；事实编辑和 Automation participation 遵守项目治理角色，本地绑定属于当前设备上的个人 Workspace Control。
4. owner、admin 和 member 只要能访问该项目，都可以使用本地绑定选择器；选择器列出已有本地项目，并在同一选择器内提供“添加本地项目”。新目录添加成功后立即成为当前可选项，用户可直接完成绑定。
...
项目详情的成员区向项目 owner/admin 显示“从组织添加成员”，与“生成项目邀请”并列。个人项目无此入口；组织成员列表不可用时入口说明“组织成员暂不可用”并提供重新加载。组织管理员若不是项目管理员，只能查看已有事实与所需角色说明。

| 状态 | 内容与动作 | 结果和恢复 |
|---|---|---|
| 加载 | 显示目标组织、项目和候选加载状态，添加不可用 | 完整读取组织成员与项目成员；失败保留详情，提供重试 |
| 选择 | 搜索姓名，单选成员，已加入项禁选；显示固定角色 member | 以成员关系身份提交，不要求填写编号；取消不产生写入 |
| 空结果 | 区分组织无成员、搜索无匹配、全部已加入 | 可清空搜索、刷新或关闭；不显示可提交的空选择 |
| 提交 | 展示目标成员，“正在添加”，禁止重复提交和改选 | 请求固定目标；页面其他浏览不被锁定 |
| 成功 | 显示“已加入项目”，刷新成员列表、矩阵及成员关系 | 保持当前项目；用户已有新选择时不跳回旧项目 |
| 成功但刷新失败 | 显示“成员已加入，列表同步失败” | 只提供重新加载，不把成功变为添加失败或重放写入 |
| 网络结果不明 | 显示“正在核对是否已加入”，保留目标选择 | 先重读项目成员；存在即成功，否则允许重试同一幂等添加 |
| 拒绝或对象变化 | 403 提示权限变化并停用提交；404 或目标离开组织时刷新候选 | 保留已加载只读事实；失效选择清除并解释；401 返回登录门禁 |

Sheet 使用已有弹层和灰度组件语义，焦点进入搜索框，Tab 在弹层内移动，关闭后回到入口。候选行显示姓名与组织角色以帮助识别同名成员，但项目角色固定为 member。错误文本可被辅助技术感知。提交期间关闭弹层不取消已发出的服务请求；晚到结果只更新其原始项目，不能关闭新弹层或覆盖新账户状态。

线框：[直接添加成员状态](member-add.html)。概览矩阵仍只读，成员详情通过项目导航进入上述明确项目上下文。

项目详情同时承担当前对象的就地推进引导，但不把四个独立动作包装成固定向导：

- 详情先显示当前项目最靠前且可操作的连接缺口，并说明它实际影响 Chat、Automation 或 Today 的哪项能力；其余连接保持低强调摘要。
- 缺少本地目录时直接提供“选择本地目录”；目录已经可用但项目未允许自动领取时直接提供“允许此项目自动领取”。动作成功后留在项目详情并 fresh-read 下一项事实。
- Workset 只表示该项目是否进入顶部观察范围，不是本地绑定或自动领取的前置条件；项目详情不得用“加入产品集”替代真实阻断动作。
- 创建 Organization、邀请成员仅在用户选择团队治理或当前操作确实需要成员关系时出现，不作为个人项目的准备步骤。
- 当前用户无权执行远端治理动作时保留事实解释，显示所需角色和责任方，不提供看似可点击但必然失败的主按钮；本地目录绑定不因项目角色进入该分支。
...
Today 依据产品集当前查看范围内的成员和页内局部项目选择组合当前用户最值得处理的一项动作、活动执行、人工事项和多产品连接/工作摘要；它不保存第二套 readiness 状态，也不替代各模块的事实写入。完整策略与状态投影位于 `../today-workspace/`，平台页只拥有应用壳、导航和共享产品范围。
...
反馈列表和页头显示项目范围内的未读数量，含未读更新的反馈显示可辨识提示。首次选择反馈或 fresh notification snapshot 标记当前反馈有未读回复时，系统自动重拉当前会话；页面级、详情级和沟通记录的刷新都组合读取反馈、通知与当前会话。只有消息成功投影后才请求把该反馈通知标记为已读；消息读取失败显示恢复动作，已读回写失败保留未读提示，不清空或隐藏已加载消息。

开发者在详情中调整处理优先级、忽略、恢复为待处理、刷新、按权限删除或触发“转待办”，不能改写用户提交的反馈事实。删除先显示明确确认。已忽略且未关联待办的反馈显示“恢复为待处理”，不再显示忽略或转待办；动作无需二次确认，提交期间锁定自身。V1 通过兼容 metadata update 恢复原记录；V2 调用固定 `POST /feedbacks/{id}/restore`，由服务端在事务内重新校验当前状态和待办关联，并把同一记录原子恢复为 `pending`。只有服务端返回更新后的反馈后才替换状态和动作；失败时当前反馈、筛选、选择、滚动位置和已忽略状态保持不变。项目探测到开发者管理能力时，忽略与恢复都使用专用状态动作，转待办使用返回反馈、任务和关系的服务端原子动作。反馈已经关联待办时，详情展示待办 ID，不再允许重复转待办，也不在反馈中继续调整优先级。
...
- Feedback 处理动作失败时保留当前搜索、筛选、排序和选择，并在动作附近或全局提示中显示服务端错误；忽略恢复失败时还保留 `ignored`，不得本地乐观改为 `pending`；刷新成功后以服务端返回重新确认选择和详情。
- Feedback 消息、回复附件、通知已读、专用忽略或原子转待办失败时，只降级对应动作并保留已加载的列表、详情、消息、草稿、附件选择和详情滚动位置；同一 project/feedback 会话的并发重载只保留一个有效请求，过期或其他项目的同 ID 响应不得覆盖当前结果。401 回到登录门禁，403 保留只读事实，404 刷新当前反馈范围。
