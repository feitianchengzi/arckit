...
- Work、Feedback 与 Automation 保留既有七状态待办、Feedback V1、验收问题和单活动执行语义；“反馈”仅指产品用户反馈，研发完成后的检查项统一称为“验收问题”。
...
Work 按顶部产品集观察范围读取完整团队待办，并在面板内提供七种服务器状态筛选。状态筛选下方固定使用左侧任务列表与右侧 Inspector 双栏；进入、切换范围或切换状态时自动选中首条结果，选择列表项只更新 Inspector，不离开 Work。Inspector 展示团队待办事实和管理动作；当前用户 Automation 快照中的 completed 任务额外展示验收问题与 Composer，accepted 任务只显示验收通过。创建和编辑使用 Workshop 的真实字段；附件、标签、权限拒绝、部分成功和部分失败保持现有服务语义。
...
Feedback 按顶部产品集观察范围只展示 Workshop Feedback V1 用户反馈，不展示或统计验收问题。ArcOrbit 验收问题只属于 Automation 独立队列；V2、待办历史和事务型反馈转待办在服务能力未成立前保持 unavailable 或显式两步结果。
