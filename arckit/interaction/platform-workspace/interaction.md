...
- Feedback 使用左侧列表与右侧详情面板。搜索、状态筛选和排序限定列表；进入页面或当前选择不再命中结果时自动选择首条，选择反馈只更新详情，不离开页面。每条反馈固定为一个视觉行，在同一行展示未读、标题、产品、优先级、状态和时间，不显示摘要副行，也不因结果数量少而拉伸填满列表。
- Feedback 详情在固定工作台内独立纵向滚动，列表保留自己的选择和滚动位置。原文及沟通附件中的图片自动加载预览，点击后与 Work 评论图片使用同一独立图片窗口；非图片附件继续使用受控打开动作。
- Work 与 Feedback 共享页面级主工作区骨架：全局产品集栏之下只有一条固定高度的页面控制轨，列表与详情双栏取得其余全部可用高度。页面外层不滚动，列表正文与详情正文各自滚动，面板标题保持可见；加载、空态和错误只替换面板正文，不改变工作台几何。
- 页面控制轨不显示大标题说明区，也不换行。Work 在常规宽度显示七状态分段、搜索、统一多维筛选入口和创建动作；Feedback 显示搜索、处理状态、排序和刷新。受支持的窄窗口保留页面身份、搜索、当前状态和筛选入口，把 Work 七状态收敛为状态菜单，并把排序、刷新或其他低频动作收入“更多操作”，恢复宽度时不改变选择、筛选或滚动位置。
...
Work 按顶部产品集观察范围读取完整团队待办。全局产品集栏下方的一条页面控制轨同时承载七种服务器状态、搜索、统一多维筛选入口和“创建待办”，不再使用独立大标题、说明区或多行筛选卡片；多维条件在不参与高度计算的弹出面板中完成。控制轨下方固定使用左侧任务列表与右侧 Inspector 双栏并占满剩余高度；进入、切换范围或切换状态时自动选中首条结果，选择列表项只更新 Inspector，不离开 Work。列表正文与 Inspector 正文独立滚动。Inspector 展示团队待办事实和管理动作；当前用户 Automation 快照中的 completed 任务额外展示验收问题与 Composer，accepted 任务只显示验收通过。创建待办时，产品字段在有效单产品范围下默认选中该产品，在“项目集全部”或失效范围下回退到 Workset 首个可用产品，并允许用户提交前改选。创建和编辑使用 Workshop 的真实字段；附件、标签、权限拒绝、部分成功和部分失败保持现有服务语义。
...
Feedback 按顶部产品集观察范围展示 Workshop 用户反馈，不展示或统计验收问题。页面身份与用途在左侧导航和单行控制轨中表达，不使用占据工作高度的大标题说明区；页面不显示创建反馈按钮、编辑反馈入口、协议版本标题或版本能力提示。

单行控制轨提供正文、短 ID、用户标识、手机号和邮箱搜索，处理状态筛选，以及最新、最早和优先级排序与刷新动作；搜索占据可伸缩空间，其余控件保持稳定宽度。反馈列表紧接控制轨位于左侧，列表项显示内容摘要、产品、优先级和处理状态，并始终保持单行高度；进入页面、切换产品范围、改变筛选或删除当前项后，系统保留仍有效的选择，否则选中首条结果。无结果时列表显示与当前范围或筛选对应的空状态，详情显示未选择提示。

列表与右侧详情共同取得控制轨下方全部剩余高度。列表标题和详情标题保持可见，列表正文与详情正文分别独立滚动；滚动详情不移动左侧列表，滚动列表不移动详情。右侧详情固定展示所选反馈的短 ID、完整正文、提交时间、产品来源、用户标识、手机号、邮箱、附件、处理状态、优先级和关联待办。受限 V2 开发者能力读取成功时，详情还展示用户、开发者和系统消息及附件；开发者可以发送文本或附件回复，附件上传与读取均使用服务端签发的受限策略或凭证。V2 列表真实不可用而回退到 V1 的项目只展示已有反馈事实，不伪造沟通时间线或回复入口。

反馈原文和沟通附件中的图片进入详情后自动显示加载状态并加载预览。已加载图片可点击进入与 Work 评论图片相同的独立窗口，窗口支持缩放、适合窗口、实际大小、旋转、平移、重置和另存为。单图加载失败在原位置显示错误和重试，不清空正文、其他附件或沟通记录；非图片附件保留文件打开动作。
...
- Feedback 的加载、空态、筛选无结果、列表失败、单项失败和长沟通记录都保留单行控制轨与占满剩余高度的双栏几何；恢复动作位于对应面板正文内，不触发页面外层滚动。
...
- Feedback 图片加载或独立窗口打开失败时，只在对应图片位置提供重试；详情滚动位置、列表选择、正文和其他已加载图片保持不变。
