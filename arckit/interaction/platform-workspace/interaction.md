...
Feedback 按顶部产品集观察范围只展示 Workshop Feedback V1 用户反馈，不展示或统计验收问题。用户触发“转待办”后，系统以完整反馈正文预填待办内容，并只列出该反馈所属项目的成员名称；选中成员时提交其 user_id，保留“未分配”时提交空执行人。用户仍可在提交前调整正文、初始状态、执行人、优先级和标签。ArcOrbit 验收问题只属于 Automation 独立队列；V2、待办历史和事务型反馈转待办在服务能力未成立前保持 unavailable 或显式两步结果。
