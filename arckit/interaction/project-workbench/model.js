...
(() => {
 const view = 'desk';
 const key = 'arcorbit-interaction-project-workbench-v1' + (new URLSearchParams(location.search).get('scenarioTools') === 'on' ? '-scenarios' : '');
 const displayTitle = text => { const parts = Array.from(new Intl.Segmenter('zh-CN', {granularity:'grapheme'}).segment(text.trim().split('\n')[0]), x=>x.segment); return parts.length > 64 ? parts.slice(0,63).join('')+'…' : parts.join(''); };
 ...
 function writable(){if(state.permissionDenied)throw new Error('没有写入权限。草稿已保留，请联系项目负责人核对权限。');if(state.offline)throw new Error('当前已离线。已保留草稿；恢复连接后再提交。');if(state.failNext){state.failNext=false;save();throw new Error('提交失败，内容已保留。重试会继续处理同一件事情。');}}
 function activate(t){
