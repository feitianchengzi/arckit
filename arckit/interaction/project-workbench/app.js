(() => {
 ...
 function render({follow=false}={}){
  capture();const active=document.activeElement,actionFocus=active?.dataset.action?{action:active.dataset.action,index:active.dataset.index,task:active.dataset.task}:null,runtimeFocus=active?.closest('.runtime-status-host')?{action:active.dataset.action,task:active.dataset.task}:null,field=active?.matches('.composer textarea,#search')?{selector:active.id?'#'+active.id:'.composer textarea',owner:active.closest('form')?.dataset.owner,start:active.selectionStart,end:active.selectionEnd}:null;
  M.save();V.render();
  ...
  if(actionFocus&&!dialog.open&&!runtimeFocus){
   const candidates=[...document.querySelectorAll('[data-action="'+actionFocus.action+'"]')];
   candidates.find(el=>!el.closest('[inert]')&&el.dataset.index===actionFocus.index&&el.dataset.task===actionFocus.task)?.focus({preventScroll:true});
  }
 ...
 function close(){document.querySelector('[data-action=filters]')?.setAttribute('aria-expanded','false');if(dialog.open)dialog.close();dialog.innerHTML='';dialog.className='';if(returnFocus?.isConnected)returnFocus.focus({preventScroll:true});else if(returnFocus?.dataset.action){const selector='[data-action='+CSS.escape(returnFocus.dataset.action)+']';[...document.querySelectorAll(selector)].find(el=>!el.closest('[inert]'))?.focus({preventScroll:true});}}
 function open(title,body,footer='',form=''){
  ...
  if(form==='filters')document.querySelector('[data-action=filters]')?.setAttribute('aria-expanded','true');if(!dialog.open)dialog.showModal();setTimeout(()=>dialog.querySelector('textarea,input:not([type=checkbox]),select')?.focus(),0);
 }
 ...
 function handle(action,el){
  ...
  switch(action){
   ...
   case 'reconcile-version':
    dialog.dataset.revision=t.revision;
    dialog.querySelector('.form-error').textContent='已核对当前事实：'+t.body+'；当前状态：'+M.states[t.status]+'。你的输入保持不变，请确认后重新提交。';
    break;
   ...
   case 'stop':open('停止执行',`<p>停止“${esc(t.title)}”的 Auto，保留已经完成的工作。事情不会被标记为完成或取消。</p>`,cancel()+button('confirm-stop','停止执行','destructive'));break;
 ...
 function showError(error){const el=dialog.open?dialog.querySelector('.form-error'):null;if(el){
   el.textContent=error.message;
   const current=M.state.tasks.find(x=>x.id===dialog.dataset.owner);
   if(current&&current.revision!==Number(dialog.dataset.revision)){
    const reconcile=document.createElement('button');reconcile.type='button';reconcile.dataset.action='reconcile-version';reconcile.textContent='核对最新版本';el.append(reconcile);
   }
 ...
 document.addEventListener('submit',event=>{
  ...
  try{
   switch(form.dataset.form){
    ...
    case 'edit':{M.writable();if(t.revision!==Number(dialog.dataset.revision))throw new Error('事情有了新的进展。你的修改已保留，请核对最新版本后确认是否重新提交。');const body=String(data.get('body')).trim();if(!body)throw new Error('目标不能为空。');const changed=t.assignee!==data.get('assignee')||t.status!==data.get('status');if(t.body!==body){D.ensure(t);t.criteria.forEach(x=>x.checked=false);}t.body=body;t.title=M.displayTitle(body);t.assignee=data.get('assignee');t.status=data.get('status');t.priority=data.get('priority');t.tag=data.get('tag');if(changed&&M.ownsLane(t)){t.mode='failed';M.record(t,'归属或状态变化，执行暂停等待核对');}else if(changed)t.mode=t.status==='accepted'?'accepted':t.status==='completed'?'done':'manual';M.record(t,'你更新了事情内容与属性',{actor:'我',kind:'decision'});close();render();toast('已保存修改。');break;}
 ...
 document.addEventListener('keydown',event=>{if(event.isComposing||composing)return;if(event.key==='Enter'&&(event.metaKey||event.ctrlKey)&&event.target.closest('[data-form=message]')){event.preventDefault();event.target.closest('form').requestSubmit();}if(event.key==='Escape'&&!dialog.open){if(M.state.runOpen){event.preventDefault();closeRuntime(true);}else if(M.state.listOpen){M.state.listOpen=false;render();}else if(M.state.surface==='chat'){event.preventDefault();closeChat();}}});
