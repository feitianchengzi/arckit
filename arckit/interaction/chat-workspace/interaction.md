...
- 右侧会话列表按 Product Workspace 分组显示会话。项目组标题包含工作区名称、可用状态和会话数量；组内会话包含标题、最近时间、活动/中断/失败状态和更多菜单。
...
- Composer 位于中间对话区底部，包含多行输入、当前会话的 Model/Level 快捷输入、发送或停止主按钮、快捷键说明和当前权限边界摘要。
...
- 会话只按所属 Product Workspace 分组，不受页面或平台全局产品范围过滤。项目组按组内最新 `updated_at` 倒序排列；时间相同时以工作区名称和稳定 id 保持确定顺序。
- 组内会话按 `updated_at` 倒序排列；时间相同时以稳定 session id 排序。运行、等待批准、中断和失败通过状态标记表达，不把会话移出所属项目组。
- 项目组默认只渲染前 10 个会话。总数超过 10 时，组底部显示“查看历史会话（其余 N 个）”；触发后在原组内展示完整列表并将入口改为“收起历史会话”。展开或收起不改变当前会话、transcript、草稿或 turn。
- 当前选中会话位于默认 10 个之外时，对应项目组保持展开，使选中项在右侧可见；切换到其他项目的最近会话后，该组可以恢复默认折叠状态。
...
依据用户 2026-09-17 的要求，default.html 迁移为完整可操作页面，中央对话在剩余空间中伸缩，消息和 Composer 保持最多 760px 的居中阅读宽度；会话列表在右侧独立滚动。760px 及以下右侧列表从按钮打开抽屉，选择后收起；Esc 关闭并恢复焦点，不停止 turn。新建仍在列表底部。
