...
核心任务包括：按项目浏览最近与历史会话、建立或恢复会话、确认新会话的项目归属、提交消息、阅读流式回答、理解工具与权限状态、随时停止生成、从失败或重启中恢复、重命名或删除会话。页面使用双栏结构，左栏只管理自由 Chat session，右栏同时容纳 transcript 和 Composer，不提供后续对象转换 Inspector。右栏宽度始终服从页面父容器，消息中的超长结构化内容不改变页面宽度。

## 页面结构

- 顶部显示 `Personal / Chat`、Codex 连接状态和新建对话按钮，不用全局 Product Workspace 选择器过滤会话列表。
- 左栏按 Product Workspace 分组显示会话。项目组标题包含工作区名称、可用状态和会话数量；组内会话包含标题、最近时间、活动/中断/失败状态和更多菜单。
...
- 当前选中会话位于默认 10 个之外时，对应项目组保持展开，使选中项在左栏可见；切换到其他项目的最近会话后，该组可以恢复默认折叠状态。
...
- 新对话始终在右栏标题区和 Composer 边界摘要中显示“会话属于：{Product Workspace}”。该控件在首条消息发送前可操作，并列出所有已绑定本地目录且 Setup Readiness 可用的工作区。
...
- Chat 页面、右栏、transcript 和消息的横向边界只由各自父容器决定；JSON、代码和表格查看器内部滚动不产生页面级横向滚动。
