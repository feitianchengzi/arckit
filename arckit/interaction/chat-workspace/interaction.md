...
核心任务包括：按项目浏览最近与历史会话、建立或恢复会话、确认新会话的项目归属、提交消息、阅读流式回答、理解工具与权限状态、随时停止生成、从失败或重启中恢复、重命名或删除会话。页面使用左侧应用导航、中间对话、右侧会话列表的三栏结构。右侧只管理自由 Chat session，中间同时容纳 transcript 和 Composer，不提供后续对象转换 Inspector。中间对话区宽度始终服从页面父容器，消息中的超长结构化内容不改变页面宽度。

## 页面结构

- 左侧应用导航保持 PERSONAL、PRODUCT、PRODUCT LIFECYCLE、ORGANIZATION 四组，Chat 与 Thing 并列；个人中心复用完整账户与 Runtime 能力。
- 顶部显示 `Chat` 和 Codex 连接状态，新建对话入口位于右侧会话列表底部，不用全局 Product Workspace 选择器过滤会话列表。
...
- Composer 位于中间对话区底部，包含多行输入、当前会话的 Model/Level 列表选择、发送或停止主按钮、快捷键说明和当前权限边界摘要。
...
- 会话只按所属 Product Workspace 分组，不受页面或平台全局产品范围过滤；项目保持稳定 id 顺序，不因会话活动、选中或项目名称变化重排。
- 组内会话保持创建时间升序，同时间使用稳定 session id，新会话追加。活动、选中、重命名和刷新不改变既有相对位置。
- 项目默认显示前 5 条；有剩余时显示“查看更多（剩余 N 个）”，每次增加 5 条，全部显示后隐藏入口。项目状态独立，收起时重置显示数量。
- 选中或活跃会话不强制展开项目、不自动增加显示数量；收起项目不改变当前对话、草稿和运行中的 turn。
- 只有拥有自由 Chat session 的项目出现在分组列表中；没有历史的可用工作区仍可从新对话的目标工作区选择器进入。工作区暂不可用时，已有会话组保留可读并标记不可用，但不允许在该工作区新建或继续发送。
...
- 输入框附近显示 Model 与 Level 列表选择控件，不提供输入过滤。新对话继承 Chat 默认配置，既有会话恢复保存值。Model 清单来自 Codex；Level 随模型提供的能力更新，切换后旧 Level 不支持时选择 medium（若可用）或首项。清单获取失败保留已有配置与发送能力，不制造可选能力。
...
使用 visual/_library/generated-tokens.css、brief.md、components/conversation.yaml 和 light 主题的既有浅色与浅杏橙角色。旧静态状态稿保留为 states.html 辅助说明，不再承担主入口。scripts 按模型、渲染与行为拆分，避免把完整入口拆为静态骨架。

原型仅操作隔离本地样本，真实 Codex、Workspace 目录选择、认证与后台同步由生产验证。场景工具提供离线、失败、权限、工作区缺失、长消息、清单不可用和恢复路径；不在产品区域添加演示控件。生产布局及 spec 已同步右侧会话与中央对话；生产 Electron 验证见 `arckit/cases/evidence/CASE-20260917-001/chat-electron.json`，仍使用真实会话协调器与 IPC。
