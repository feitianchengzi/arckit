...
app.whenReady().then(async()=>{
 ...
 try {
  ...
  assert.ok(geometry.nav<=geometry.centerLeft&&geometry.centerRight<=geometry.list&&geometry.reading>=geometry.centerRight-geometry.centerLeft-60);
  assert.equal(await run('document.querySelectorAll(".session-group:first-child .session-row").length'),5);
  await click('[data-chat-action=history][data-project=atlas]');assert.equal(await run('document.querySelectorAll(".session-group:first-child .session-row").length'),10);await click('[data-chat-action=history][data-project=atlas]');assert.equal(await run('document.querySelectorAll(".session-group:first-child .session-row").length'),12);
  await click('[data-chat-action=project][data-project=atlas]');assert.equal(await run('document.querySelectorAll(".session-group:first-child .session-row").length'),0);
  await click('[data-chat-action=project][data-project=atlas]');assert.equal(await run('document.querySelectorAll(".session-group:first-child .session-row").length'),5);
  await click('[data-chat-action=history][data-project=atlas]');await click('[data-chat-action=history][data-project=atlas]');
  await run(`document.querySelector('[data-resize="width"]').dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowLeft',bubbles:true}));document.querySelector('[data-resize="height"]').dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowUp',bubbles:true}))`);
  assert.equal(await state('M.state.sidebarWidth'),320);assert.equal(await state('M.state.inputHeight'),110);
  await shot('chat-desktop');checks.push('Four navigation groups; Chat centered between app navigation and right session list; Full reading width; history increases by five');
  ...
  await input('chat-level','medium');await submit();assert.equal(await state('M.state.sessions.length'),count+1);assert.equal(await state('M.current().project'),'borealis');
  const created=await state('M.current().id'),thread=await state('M.current().thread');
  await tick();await input('chat-level','high');assert.equal(await state('M.current().turnConfig.level'),'medium');
