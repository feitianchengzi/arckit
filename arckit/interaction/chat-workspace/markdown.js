/* Local browser projection adapted from the repository restricted Markdown renderer. */
(() => {
function renderRestrictedMarkdown(value = "") {
...
function renderLink(label, href) {
  ...
  try {
    ...
    if (!['http:', 'https:'].includes(url.protocol)) throw Error('unsupported');
    return `<a href="${escapeHtml(url.href)}" target="_blank" rel="noopener">${safeLabel}</a>`;
  } catch { return `${safeLabel} <code>${escapeHtml(href)}</code>`; }
}
...
window.ChatMarkdown = renderRestrictedMarkdown;
})();
