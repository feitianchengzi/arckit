...
<html lang="zh-CN">
  ...
  <body>
    ...
    <!-- generated at {now} by feedback-sdk-web deploy -->
  </body>
</html>
"""
...
def upload_spa_aliases(bucket: oss2.Bucket, dist_dir: Path, prefix: str):
    ...
    for object_key in [prefix, f"{prefix}/submit", f"{prefix}/status"]:
        bucket.put_object(object_key, index_html, headers=HTML_CACHE_HEADERS)
        log(f"  uploaded SPA alias: {object_key} ({format_size(len(index_html))})")
