  ...
  return (
    <section className="space-y-5">
      <header>
        ...
        <h2 className="mt-1 text-xl font-bold text-foreground">{t('form.title')}</h2>
        <p className="mt-1 text-sm text-foreground-secondary">{t('form.subtitle')}</p>
      </header>
      ...
      <div className="rounded-xl border border-dashed border-border bg-surface p-4 text-sm text-foreground-secondary transition-colors hover:border-border-hover">
        <div className="flex items-start justify-between gap-3">
          <div>
            <span className="block text-sm font-semibold text-foreground">{t('form.screenshot_optional')}</span>
            <span className="mt-1 block text-xs text-foreground-tertiary">{t('form.screenshot_hint')}</span>
          </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-foreground-secondary">
          ...
          >
            {images.length ? t('form.add_more') : t('form.pick_image')}
          </button>
          <span>{images.length ? t('form.selected_count', { count: images.length, max: maxImages }) : t('form.max_count', { max: maxImages })}</span>
        </div>
        {images.length ? (
              ...
              <div key={image.id} className="min-w-0">
                <div className="group relative aspect-square overflow-hidden rounded-xl border border-divider bg-surface-elevated shadow-sm">
                  <button
                    ...
                    aria-label={t('form.view_large', { name: image.name })}
                  >
                  ...
                  <button
                    ...
                    aria-label={t('form.remove_image', { name: image.name })}
                    title={t('form.remove_title')}
                  >
                    ×
                  </button>
                  {image.status === 'uploading' ? (
                    <div className="absolute inset-x-0 bottom-0 bg-black/65 px-2 py-1.5 text-center text-[11px] font-semibold text-white">
                      {t('form.uploading', { percent: Math.round(image.progress * 100) })}
                    </div>
                  ) : image.status === 'error' ? (
                    <div className="absolute inset-x-0 bottom-0 bg-error/90 px-2 py-1.5 text-center text-[11px] font-semibold text-white">{t('form.upload_failed')}</div>
                  ) : (
                    <div className="absolute bottom-1.5 left-1.5 rounded-full bg-success/90 px-1.5 py-0.5 text-[10px] font-semibold text-white">{t('form.upload_done')}</div>
                  )}
                </div>
      ...
      {uploading ? (
        <div className="rounded-lg bg-primary-lighter px-3 py-2 text-xs text-primary">
          {t('form.uploading_n', { count: uploadingImages.length, percent: Math.round(averageProgress * 100) })}
        </div>
      ) : null}

      {!uploading && uploadedCount > 0 && uploadedCount === images.length ? (
        <div className="rounded-lg bg-success-light px-3 py-2 text-xs text-success">{t('form.upload_complete', { count: uploadedCount })}</div>
      ) : null}
      ...
      {previewImage ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={t('form.preview_label', { name: previewImage.name })}
        ...
        >
          <div className="relative flex max-h-[92dvh] max-w-[94vw] flex-col items-center" onClick={(event) => event.stopPropagation()}>
            <button
              ...
              aria-label={t('form.close_preview')}
              title={t('form.close_preview')}
            >
              ×
            </button>
            <img
